...
knitr::opts_knit$set(root.dir = normalizePath("C:/Users/Sara/Documents/GitHub/green-crab-metabolomics/")) #Set root directory
```
...
```{r}
source('C:/Users/Sara/Documents/GitHub/green-crab-metabolomics/code/biostats.R') #Source multivariate analysis functions
```
...
```{r Comparison between tanks across individual time points - carapace width, echo=FALSE}
...
```{r Comparison between tanks across individual time points - carapace length, echo=FALSE}
#####
...
```{r Comparison between tanks across individual time points - weight, echo=FALSE}
########
...
```{r Comparison between tanks across individual time points - integument color, echo=FALSE}
#########
...
```{r Combined Treatments Over Time - carapace width, echo=FALSE}
...
```{r Combined Treatments Over Time - carapace length, echo=FALSE}                            
##################
...
```{r Combined Treatments Over Time - weight, echo=FALSE}
##################
...
```{r Combined Treatments Over Time - integument color, echo=FALSE}
##################
...
```{r Comparison between treatments across individual time points - carapace width, echo=FALSE}
setwd("C:/Users/Sara/Documents/GitHub/green-crab-metabolomics/data/")
...
```{r Comparison between treatments across individual time points - carapace length, echo=FALSE}
setwd("C:/Users/Sara/Documents/GitHub/green-crab-metabolomics/data/")
...
```{r Comparison between treatments across individual time points - weight, echo=FALSE}
setwd("C:/Users/Sara/Documents/GitHub/green-crab-metabolomics/data/")
...
```{r Comparison between treatments across individual time points - integument color, echo=FALSE}
setwd("C:/Users/Sara/Documents/GitHub/green-crab-metabolomics/data/")
...
dat_all <- crab_dat %>%
  mutate(day = recode(date, "7/8/2022" = 4, "7/12/2022" = 8, "7/15/2022" = 11, "7/19/2022" = 15, "7/22/2022" = 18, "7/26/2022" = 22)) %>%
  mutate(temperature = recode(treatment.tank, "1" = 14, "2" = 6, "3" = 31, "4" = 14, "5" = 6, "6" = 31)) %>% mutate(sex = recode(sex, "F"=1, "M"=2)) %>%
...
head(dat_all)
dat_wl <- dat_all%>%mutate(width_length = carapace.width/carapace.length) 
head(dat_wl)
dat_wl <- dat_wl[c(1:260), c(3:8)]
head(dat_wl)
...
```{r PCA, width/length}
pca_res_crab_wl <- prcomp(dat_wl, scale. = TRUE) #Conduct PCA
summary(pca_res_crab_wl) #Standard deviations for each PC
```

```{r PCA interpretation, width/length}
pca.eigenval(pca_res_crab_wl) #Eigenvalues greater than the broken-stick expectation are considered to explain a statistically significance proportion of the variance in the original dataset. PC1 eigenvalue > BSV
ordi.monte(dat_wl, ord = "pca", dim = 6, plot = FALSE) #Conduct monte carlo randomization test using 1000 random permutations of the input data. PC1 and PC2 p-values < 0.05

#Support exists for both PC1 and PC2
```

```{r PCA interpretation cont., width/length}
pca_res_crab_wl$rotation #Correlations between individual variables and PCs
pca.structure(pca_res_crab_wl, dat_wl, dim = 5, cutoff = 0.4) #Structure coefficients between variables and PCs above the given cutoff
```

```{r PCA plot, width/length}
dat_wl$temperature <- as.factor(dat_wl$temperature)
head(dat_wl)

p <- autoplot(pca_res_crab_wl, data = dat_wl, colour = "temperature", size = 2, loadings = TRUE, loadings.colour = 'purple', loadings.label = TRUE, loadings.label.size = 5, loadings.label.colour = 'black')

p2 <- p + scale_color_manual(values = c("#2171B5","#525252","#CB181D")) + theme(legend.text = element_text(size = 15)) + theme(legend.title = element_text(size = 15)) + stat_ellipse(geom = "polygon", aes(fill = temperature), alpha = 0.25) + guides(shape = "none") + scale_fill_manual(values = c("#2171B5","#525252","#CB181D"), labels=c("Cold 6C", "Ambient 14C", "Hot 31C")) + labs(fill = "Temperature (C)") + guides(color = "none") + ggtitle("PCA, Width/Length")

p2

```

```{r PCA plot, width and lenght separate}
...
dat_all$temperature <- as.factor(dat_all$temperature)
head(dat_all)

p_all <- autoplot(pca_res_crab, data = dat_all, colour = "temperature", size = 2, loadings = TRUE, loadings.colour = 'purple', loadings.label = TRUE, loadings.label.size = 5, loadings.label.colour = 'black')

p2_all <- p_all + scale_color_manual(values = c("#2171B5","#525252","#CB181D")) + theme(legend.text = element_text(size = 15)) + theme(legend.title = element_text(size = 15)) + stat_ellipse(geom = "polygon", aes(fill = temperature), alpha = 0.25) + guides(shape = "none") + scale_fill_manual(values = c("#2171B5","#525252","#CB181D"), labels=c("Cold 6C", "Ambient 14C", "Hot 31C")) + labs(fill = "Temperature (C)") + guides(color = "none") + ggtitle("PCA, Width and Length Separate")

p2_all
```

```{pca practice..}
#p <- autoplot(pca_res_crab, data = dat_all, colour = 'temperature', shape = 'temperature', loadings = TRUE, loadings.colour = 'blue', loadings.label = TRUE, loadings.label.size = 3, frame = TRUE, frame.type = 'norm', frame.colour = 'temperature')

#p2 <- p + scale_color_manual(values = c("#2171B5","#525252","#CB181D")) + theme(legend.text = element_text(size = 15)) + theme(legend.title = element_text(size = 15)) + stat_ellipse()

#ggplotly(p2) 

#p_ <- autoplot(pca_res_crab, data = dat_all, colour = 'temperature', shape = 'temperature', loadings = TRUE, loadings.colour = 'blue', loadings.label = TRUE, loadings.label.size = 3, ellipse = TRUE,ellipse.level = 0.95,ellipse.type = "norm",ellipse.alpha = 0.1,ellipse.border.remove = FALSE)

#p_2 <- p_ + scale_color_manual(values = c("#2171B5","#525252","#CB181D")) + theme(legend.text = element_text(size = 15)) + theme(legend.title = element_text(size = 15))

#ggplotly(p_2)

#, frame.level = .95, show.legend = FALSE
# colby = 'temperature', colkey = c('6' = "#2171B5", '14' = "#525252", '31' = "#CB181D"), colLegendTitle = 'Temperature', 
# + stat_ellipse(type = 'norm', level = 0.95)
#frame = TRUE, frame.type = 'norm', frame.prob = .95
#frame = TRUE, frame.type = 'norm'
#+ scale_fill_discrete(name = "Temperature", labels = c("6C", "14C", "31C"))
#ggplotly(p) + scale_color_manual(values = c("#2171B5","#525252","#CB181D"))
#factor(dat_all$temperature)
#col.hab = c("#2171B5","#525252","#CB181D"),
#col.var = c("#2171B5","#525252","#CB181D") ,
#pca_res_crab_i <- data.frame(pca_res_crab$x, temperature=dat_all$temperature)



#pca

#my_colors <- c("#2171B5","#525252","#CB181D")
#names(my_colors) <- levels(factor(c(levels(dat_all$temperature))))
#my_scale <- scale_fill_manual(name = "temperature", values = my_colors)
...
############################library(data.table)
#dat_all <- dat_all%>%mutate(width_length = carapace.width/carapace.length)
#head(dat_all)
#dat_all$temperature <- as.factor(dat_all$temperature)
#pca_res_crab_i <- data.frame(pca_res_crab$x, temperature=dat_all$temperature)

#pca <- ggplot(data = pca_res_crab_i, aes(x = PC1, y = PC2, col = temperature)) + geom_point(size=2) + scale_color_manual(values = c("#2171B5","#525252","#CB181D")) + stat_ellipse(type = 'norm', level = 0.95)

#pca
#dat_all$width_length <- tapply(dat_all$carapace.width, dat_all$carapace.length, function(x) as.numeric(factor(x)))
...
#dat_all <- crab_dat %>%
  #mutate(day = recode(date, "7/8/2022" = 4, "7/12/2022" = 8, "7/15/2022" = 11, "7/19/2022" = 15, "7/22/2022" = 18, "7/26/2022" = 22)) %>%
  #mutate(temperature = recode(treatment.tank, "1" = 14, "2" = 6, "3" = 31, "4" = 14, "5" = 6, "6" = 31)) %>% mutate(sex = recode(sex, "F"=1, "M"=2)) %>%
  #select(., -c(carapace.width, carapace.length,treatment.tank, date)) #Make recode dates to days and create a new column for temperature. Drop date and treatment.tank
#head(dat_all)
```