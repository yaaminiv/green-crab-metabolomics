...
```{r}
all_crab %>%
  select(., day, treatment, carapace.length) %>%
  filter(., day == 1 | day == 22) %>%
  ggplot(., aes(x = treatment, y = carapace.length, color = treatment, shape = treatment)) +
  geom_boxplot() + geom_point() + 
  facet_wrap( ~ day, labeller = labeller(day = facet_labels)) +
  stat_compare_means() +
  ylim(25, 50) +
  ...
  theme_classic(base_size = 15) + theme(axis.ticks.x = element_blank(),
                                        axis.title.x = element_blank(),
                                        strip.text.x = element_text(size = 15)) #Select day 1 and day 22 data for factor of interest. Plot boxplot and points with treatment on the x-axis and factor on the y-axis, with shape and color defined  by treatment. Facet wrap by day and add specific facet labels. Add stats for comparisons between means and modify y-axis limits to fit statistical testing output at the top. Relabel x and y axes. Remove x-axis labels. Manually add color and shape legend. Set theme and modify bckground colors and strip text size.
ggsave("figures/carapace-length-comparison.pdf", height = 8.5, width = 11)
```
...
```{r}
signif(integumentColorDay1$pvalDay1[1], 2)
signif(integumentColorDay22$pvalDay22[1], 1)
```
...
```{r}
signif(sexDay1$pvalDay1[1], 2)
signif(sexDay22$pvalDay22[1], 2)
```

```{r}
sex_text <- data.frame(label = c("Chi-squared, p = 0.96", "Chi-squared, p = 0.83"),
                                   day = c(1, 22),
                                   x = c(1.42, 1.42), 
                                   y = c(1.07, 1.07))
```
...
all_crab %>%
  ...
  scale_x_discrete(breaks = c("05", "13", "30"),
                   labels = c("5ºC ", "13ºC ", "30ºC")) +
  labs(fill = "Sex") +
  geom_text(data = sex_text, mapping = aes(x = x, y = y, label = label), inherit.aes = FALSE) +
  theme_classic(base_size = 15) + theme(axis.ticks.x = element_blank(),
                                        axis.title.x = element_blank(),
...
```{r}
integumentColor_textMultipanel <- data.frame(label = c("Chi-squared, p = 0.74", "Chi-squared, p = 0.003"),
                                   day = c(1, 22),
                                   x = c(1.65, 1.7), 
                                   y = c(1.08, 1.08))
```
...
integumentColorFig <- all_crab %>%
  ...
  scale_fill_manual(name = "Integument Color",
                    values = c("darkturquoise", "#66C2A5", "#A6D96A", "#D9EF8B", "#FFFF99", "#FDAE61", "#F46D43", "#D53E4F"),
                    labels = c("B", "BG", "G", "YG", "Y", "YO", "O", "RO")) +
  geom_text(data = integumentColor_textMultipanel, mapping = aes(x = x, y = y, label = label), inherit.aes = FALSE) +
  theme_classic(base_size = 15) + theme(axis.ticks.x = element_blank(),
                                        axis.title.x = element_blank(),
                                        strip.text.x = element_text(size = 15),
...
```{r}
sex_textMultipanel <- data.frame(label = c("Chi-squared, p = 0.96", "Chi-squared, p = 0.83"),
                                   day = c(1, 22),
                                   x = c(1.65, 1.65), 
                                   y = c(1.08, 1.08))
```
...
sexFig <- all_crab %>%
  ...
  scale_x_discrete(breaks = c("05", "13", "30"),
                   labels = c("5ºC ", "13ºC ", "30ºC")) +
  labs(fill = "Sex") +
  geom_text(data = sex_textMultipanel, mapping = aes(x = x, y = y, label = label), inherit.aes = FALSE) +
  theme_classic(base_size = 15) + theme(axis.ticks.x = element_blank(),
                                        axis.title.x = element_blank(),
                                        strip.text.x = element_text(size = 15),
                                        legend.position = "bottom",
