...
Before I make the figures, I want to determine if there are statistical differences for the variable of interest between temperatures at each timepoint (day 1 and day 22). Since crabs were measured on both days and do not have IDs, I need to account for repeated measures. 

#### Format data

```{r}
#Convert certain variables to factors
all_crab$treatment_factor <- as.factor(all_crab$treatment)
all_crab$day_factor <- as.factor(all_crab$day)
all_crab$treatment.tank_factor <- as.factor(all_crab$treatment.tank)
all_crab$sex_factor <- as.factor(all_crab$sex)
```

```{r}
crab_1_22 <- all_crab %>%
  filter(., day == 1 | day == 22) #Reduced dataset with the first and last days only
```

#### Carapace width

```{r}
carapaceWidthLM <- lmer( #Linear model
  carapace.width ~ (treatment_factor)*(day_factor) + #Carapace width differences assessed based on treatment*day
                          (1 | treatment.tank_factor), #Tank as a random effect
                        data = crab_1_22) #Dataset used
anova(carapaceWidthLM) #No variable significantly impacts carapace width!
```

```{r}
# Diagnostic plots: Check for constant variance and normal residuals
plot(carapaceWidthLM) # Fitted vs Residuals
qqnorm(residuals(carapaceWidthLM)); qqline(residuals(carapaceWidthLM))

#Residuals are pretty normally distributed - no assumptions violated!
```

#### Weight

```{r}
weightLM <- lmer( #Linear model
  log(weight) ~ (treatment_factor)*(day_factor) + #Weight differences assessed based on treatment*day
                          (1 | treatment.tank_factor), #Tank as a random effect
                        data = crab_1_22) #Dataset used
anova(weightLM) #No variable significantly impacts weight!
```

```{r}
# Diagnostic plots: Check for constant variance and normal residuals
plot(weightLM) # Fitted vs Residuals
qqnorm(residuals(weightLM)); qqline(residuals(weightLM))
...
```{r}
crab_1_22$integument_ord <- factor(crab_1_22$integument.color, 
                                   levels = c("B", "BG", "G", "YG", "Y", "YO", "O", "RO"), 
                                   ordered = TRUE) # Define integument color as an ordered factor (using original category names)
```


```{r}
integumentColorLM <- clm( #Cumulative link mixed model for ordinal data
  integument_ord ~ treatment_factor * day_factor, #Impact of temp and day
                   data = crab_1_22) #Dataset used
anova(integumentColorLM) #All predictors impact integument color
```

```{r}
# Check the proportional odds assumption
nominal_test(integumentColorLM) #Test nominal effects (tests if slopes differ across thresholds). P-value is not significant: assumption holds

scale_test(integumentColorLM) # Test scale effects (tests if variance differs across predictor levels). P-values are not significant: assumption holds.
```

```{r}
integumentRes <- resids(integumentColorLM) # Calculate surrogate residuals for integument color
qqnorm(integumentRes); qqline(integumentRes) # Residuals are normally distributed!
```
...
```{r}
sexLM <- glmer(
  sex_factor ~ treatment * day + #Impact of temp and day on sex
                 (1 | treatment.tank), #Tank as a random effect
                   family = binomial(link = "logit"), #Binomial model with logit link to deal with binary data
                   data = crab_1_22) #Dataset used
```

```{r}
car::Anova(sexLM, type = "III") #No impact of any variable on sex
```

```{r}
sim_residuals <- simulateResiduals(fittedModel = sexLM, n = 1000, plot = TRUE) #Simulate residuals for the final model
...
ggsave("figures/carapace-width-comparison.pdf", height = 8.5, width = 11)
```
...
```{r}
all_crab %>%
  select(., day, treatment, weight) %>%
  filter(., day == 1 | day == 22) %>%
  ggplot(., aes(x = treatment, y = weight, color = treatment, shape = treatment)) +
  ...
  theme_classic(base_size = 15) + theme(axis.ticks.x = element_blank(),
                                        axis.title.x = element_blank(),
                                        strip.text.x = element_text(size = 15)) #Select day 1 and day 22 data for factor of interest. Plot boxplot and points with treatment on the x-axis and factor on the y-axis, with shape and color defined  by treatment. Facet wrap by day and add specific facet labels. Add stats for comparisons between means and modify y-axis limits to fit statistical testing output at the top. Relabel x and y axes. Remove x-axis labels. Manually add color and shape legend. Set theme and modify bckground colors and strip text size.
ggsave("figures/weight-comparison.pdf", height = 8.5, width = 11)
```
...
```{r}
integumentColorDay22 <- all_crab %>%
  select(., day, treatment, integument.cont) %>%
  filter(., day == 22) %>%
  arrange(day, treatment, integument.cont) %>%
  group_by(day, treatment, integument.cont) %>%
  ungroup %>%
  summarize(., pvalDay22 = chisq.test(x = treatment, y = as.factor(integument.cont))$p.value) %>%
  as.data.frame() #Take day 22 data and arrange and group by relevant variables. Ungroup and use summarize to perform a chi-squared test examining differences in integument color by treatment. Extract p-value and save as a dataframe
integumentColorDay22
```
...
ggsave("figures/integument-color-comparison.pdf", height = 8.5, width = 11)
```
...
```{r}
sexDay1 <- all_crab %>%
  select(., day, treatment, sex) %>%
  filter(., day == 1 ) %>%
  arrange(day, treatment, sex) %>%
  group_by(day, treatment, sex) %>%
  ungroup %>%
  summarize(., pvalDay1 = chisq.test(x = treatment, y = as.factor(sex))$p.value) %>%
  as.data.frame() #Take day 1 data and arrange and group by relevant variables. Ungroup and use summarize to perform a chi-squared test examining differences in sex by treatment. Extract p-value and save as a dataframe
sexDay1
```
...
sexDay22 <- all_crab %>%
  ...
  group_by(day, treatment, sex) %>%
  ungroup %>%
  summarize(., pvalDay22 = chisq.test(x = treatment, y = as.factor(sex))$p.value) %>%
  as.data.frame() #Take day 22 data and arrange and group by relevant variables. Ungroup and use summarize to perform a chi-squared test examining differences in sex by treatment. Extract p-value and save as a dataframe
sexDay22
```
...
all_crab %>%
  ...
  group_by(day, treatment, sex) %>%
  summarize(., count = n()) %>%
  ggplot(., aes(x = treatment, y = count, fill = as.factor(sex))) +
  geom_bar(position = "fill", stat = "identity") +
  facet_wrap( ~ day, labeller = labeller(day = facet_labels)) +
  labs(y = "Percent of Crabs") + coord_cartesian(ylim = c(0, 1.06)) +
  scale_x_discrete(breaks = c("05", "13", "30"),
  ...
  theme_classic(base_size = 15) + theme(axis.ticks.x = element_blank(),
                                        axis.title.x = element_blank(),
                                        strip.text.x = element_text(size = 15)) #Select day, treatment, and sex for days 1 and 22. Arrange and group by selected variables. Summarize by grouped variables to get counts. Plot counts as a stacked barplot. Modify labels and specify a color scale.
ggsave("figures/sex-comparison.pdf", height = 8.5, width = 11)
```

#### Stitch panels together

I want to make a multipanel figure with carapace width, weight, integument color, and sex ratio. I'm excluding carapace length since carapace width is generally used for analyses.

```{r}
carapaceWidthFig <- all_crab %>%
  select(., day, treatment, carapace.width) %>%
  filter(., day == 1 | day == 22) %>%
  ggplot(., aes(x = treatment, y = carapace.width, color = treatment, shape = treatment)) +
  geom_boxplot() + geom_point() + 
  facet_wrap( ~ day, labeller = labeller(day = facet_labels)) +
  ylim(35, 60) +
  labs(x = "", y = "Carapace Width (mm)", title = "A") +
  scale_x_discrete(labels = c("5ºC", "13ºC", "30ºC")) +
  scale_color_manual(values = c(plotColors[3], plotColors[2], plotColors[1]), 
                     guide = "none") +
  scale_shape_manual(values = c(15, 19, 17),
                     guide = "none") +
  theme_classic(base_size = 15) + theme(axis.ticks.x = element_blank(),
                                        ...
                                        plot.margin = margin(t = 5, r = 20, b = 20, l = 20, 
                                                             unit = "pt"),
                                        plot.title.position = "plot") #Select day 1 and day 22 data for factor of interest. Plot boxplot and points with treatment on the x-axis and factor on the y-axis, with shape and color defined  by treatment. Facet wrap by day and add specific facet labels. Add stats for comparisons between means and modify y-axis limits to fit statistical testing output at the top. Relabel x and y axes and add a plot title. Specify x-axis labels. Specify colors and shapes but remove legend. Set theme and modify background colors and strip text size. Modify plot margins and alignment of the plot title.
carapaceWidthFig
```
...
integumentColorFig <- all_crab %>%
  ...
  geom_bar(position = "fill", stat = "identity") +
  facet_wrap( ~ day, labeller = labeller(day = facet_labels)) +
  labs(y = "Percent of Crabs", title = "C") + coord_cartesian(ylim = c(0, 1.06)) +
  scale_x_discrete(breaks = c("05", "13", "30"),
                   labels = c("5ºC ", "13ºC ", "30ºC")) +
  scale_fill_manual(name = "Integument Color",
                    values = c("darkturquoise", "#66C2A5", "#A6D96A", "#D9EF8B", "#FFFF99", "#FDAE61", "#F46D43", "#D53E4F"),
  ...
  theme_classic(base_size = 15) + theme(axis.ticks.x = element_blank(),
                                        ...
                                        plot.margin = margin(t = 5, r = 20, b = 20, l = 20, 
                                                             unit = "pt"),
                                        plot.title.position = "plot") #Select day, treatment, and integument color (continuous) for days 1 and 22. Arrange and group by selected variables. Summarize by grouped variables to get counts. Plot counts as a stacked barplot. Modify labels, add a title, and specify a color scale. Use geom_text to add specific text to each ggplot facet. Set theme and modify background colors and strip text size. Modify plot margins and alignment of the plot title.
integumentColorFig
```
...
sexFig <- all_crab %>%
  ...
  geom_bar(position = "fill", stat = "identity") +
  facet_wrap( ~ day, labeller = labeller(day = facet_labels)) +
  labs(y = "Percent of Crabs", title = "D") + coord_cartesian(ylim = c(0, 1.06)) +
  scale_x_discrete(breaks = c("05", "13", "30"),
                   labels = c("5ºC ", "13ºC ", "30ºC")) +
  labs(fill = "Sex") +
  theme_classic(base_size = 15) + theme(axis.ticks.x = element_blank(),
                                        ...
                                        plot.margin = margin(t = 5, r = 20, b = 20, l = 20, 
                                                             unit = "pt"),
                                        plot.title.position = "plot") #Select day, treatment, and sex for days 1 and 22. Arrange and group by selected variables. Arrange and group by selected variables. Summarize by grouped variables to get counts. Plot counts as a stacked barplot. Modify labels, add a title, and specify a color scale. Use geom_text to add specific text to each ggplot facet. Set theme and modify background colors and strip text size. Modify plot margins and alignment of the plot title.
sexFig
```

```{r}
(carapaceWidthFig | weightFig) / (integumentColorFig | sexFig)
ggsave("figures/multipanel-demData-comparison.pdf", height = 8.5, width = 11)
```
...
pca.structure(pca_res_crab_wl, dat_wl, dim = 3, cutoff = 0.4) #Structure coefficients between variables and PCs above the given cutoff. Simple linear correlations between original variables and PC scores. Squared structure coefficients give the percentage of variance in each original variable accounted for by each PC.
```

```{r}
write.csv(as.data.frame(pca.structure(pca_res_crab_wl, dat_wl, dim = 3, cutoff = 0.4)), "PC-structure-coefficients.csv", quote = FALSE) #Save structure coefficients
```
