...
transLipidData <- rawLipidomicsData %>%
  ...
  left_join(lipidomicsMetadata, ., by = "crab.ID") #Remove columns with MS information, convert BinBase.name to rownames, and transform dataframe. Convert rownames to crab.ID column
...
```{r}
scaled.pca <- prcomp(transLipidData[c(5:601)], scale = TRUE, center = TRUE) #Use prcomp to perform a centered and scaled PCA calculation
...
ordiMonteRandTest <- ordi.monte(transLipidData[c(5:601)], ord = "pca", dim = 6, plot = FALSE) #Conduct monte carlo randomization test using 1000 random permutations of the input data.
ordiMonteRandTest
...
```{r}
scaled.pca.id <- prcomp(transLipidData[c(4:155)], scale = TRUE, center = TRUE) #Use prcomp to perform a centered and scaled PCA calculation
summary(scaled.pca.id) #Standard deviations for each PC
```

```{r}
pcaPlot4 <- autoplot(scaled.pca.id, data = transLipidData, x = 1, y = 2,
                     size = 4, alpha = 0.8, colour = "treatment", shape = "day", 
                     frame = TRUE, frame.colour = "treatment", loadings = FALSE) +
  scale_color_manual(values = c(plotColors[3], plotColors[2], plotColors[1]),
                     name = "Temperature (ºC)") +
...
```{r}
permanova.metab <- adonis2(scale(transLipidData[c(5:601)]) ~ treatment*day, data = transLipidData, method = 'eu') #Conduct global PERMANOVA to assess significance of trends in PCA. Scale lipidomics data and assess influence of treatment, day, and treatment x day on metabolite profiles
...
```{r}
permanova.metabid <- adonis2(scale(transLipidData[c(5:156)]) ~ treatment*day, data = transLipidData, method = 'eu') #Conduct global PERMANOVA to assess significance of trends in PCA. Scale lipidomics data and assess influence of treatment, day, and treatment x day on metabolite profiles
...
The next step is for me to conduct a PLS-DA analysis using the `mixOmics` package. This is a more robust way to analyze differences between treatments and is common in many lipidomics studies. Since both temperature and day were significant in the PCA, I'll use both of those factors in the PLSDA.
...
```{r}
X <- log(transLipidData[c(5:601)]) #Assign only metabolite data to the X dataframe after log transformation.
...
cim(treatment.plsda, mapping = "Y", row.names = transLipidData$treatment, xlab = "Metabolites", ylab = "Temperature (ºC)", margins = c(8, 5), row.cex = 0.5) #Create CIM plot with metabolites on the x-axis and treatment on the y-axis. 
dev.off()
```
...
cim(treatment.plsda, mapping = "Y", row.names = transLipidData$treatment_day, xlab = "Metabolites", ylab = "Temperature (ºC) x Day", margins = c(8, 5), row.cex = 0.5) #Create CIM plot with metabolites on the x-axis and treatment x day on the y-axis. 
dev.off()
```
