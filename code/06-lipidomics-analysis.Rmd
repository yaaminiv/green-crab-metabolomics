...
VIP_select_13v30 <- cbind(clean_13v30[, 1:4], 
                                log(clean_13v30[, names(clean_13v30) %in% VIP_13v30$identifier])) %>%
  pivot_longer(., cols = 5:149, values_to = "log_norm_counts", names_to = "identifier") %>%
  group_by(., identifier) #cbind metadata columns (1:4) and log-transformed "clean" data for VIP. Pivot data longer and group by lipids.
...
t.test_13v30$p.adj <- p.adjust(t.test_13v30$p.value, method = c("fdr"), n = length(VIP_13v30$identifier)) #Adjust p value for the number of comparisons
t.test_13v30
```
...
t.test_13v30 %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>%
  nrow(.) #134 significantly different lipids
t.test_13v30 %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>% filter(., estimate1 > estimate2) %>%
  nrow(.) #99 lipids higher in 13ºC
t.test_13v30 %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>% filter(., estimate1 < estimate2) %>%
...
t.test_13v30 %>%
  filter(., p.adj < 0.05) %>%
  right_join(x = lipidIdentifiers, y = ., by = "identifier") %>%
...
t.test_13v30 %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>%
  left_join(., VIP_13v30, by = "identifier") %>%
  left_join(., lipidIdentifiers, by = "identifier") %>%
...
VIP_select_13v5 <- cbind(clean_13v5[, 1:4], 
                                log(clean_13v5[, names(clean_13v5) %in% VIP_13v5$identifier])) %>%
  pivot_longer(., cols = 5:149, values_to = "log_norm_counts", names_to = "identifier") %>%
  group_by(., identifier) #cbind metadata columns (1:4) and log-transformed "clean" data for VIP. Pivot data longer and group by lipids.
...
t.test_13v5$p.adj <- p.adjust(t.test_13v5$p.value, method = c("fdr"), n = length(VIP_13v5$identifier)) #Adjust p value for the number of comparisons
t.test_13v5
```
...
t.test_13v5 %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>%
  nrow(.) #39 significantly different lipids
t.test_13v5 %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>% filter(., estimate1 > estimate2) %>%
  nrow(.) #34 lipids higher in 13ºC
t.test_13v5 %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>% filter(., estimate1 < estimate2) %>%
  nrow(.) #5 lipids higher in 5ºC
```
...
t.test_13v5 %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>%
  left_join(., VIP_13v5, by = "identifier") %>%
  left_join(., lipidIdentifiers, by = "identifier") %>%
  mutate(., direction = case_when(estimate > 0 ~ "13",
                                  estimate < 0 ~ "5")) %>%
  mutate(., score = case_when(estimate > 0 ~ VIP,
...
X <- log(knownTransLipidData[c(5:422)]) #Assign known metabolite data to the X dataframe after log transformation
Y <- as.factor(knownTransLipidData$treatment) #Assign treatment data to the Y dataframe
```
...
PCData <- rawLipidomicsData %>%
  ...
  unite(., "lipid-identifier", lipid:identifier, sep = "-") %>%
  column_to_rownames(., var = "lipid-identifier") %>%
  t(.) %>% as.data.frame(.) %>%
  rownames_to_column(., var = "crab.ID") %>%
  left_join(lipidomicsMetadata, ., by = "crab.ID") #Slice PC data from the raw dataframe and remove extraneous columns. Unite lipid and identifier column to make unique column names. Transpose dataframe and convert crab ID to a column. Join with metadata
...
PEData <- rawLipidomicsData %>%
  ...
  t(.) %>% as.data.frame(.) %>%
  rownames_to_column(., var = "crab.ID") %>%
  left_join(lipidomicsMetadata, ., by = "crab.ID") #Slice PE data from the raw dataframe and remove extraneous columns. Unite lipid and identifier column to make unique column names. Transpose dataframe and convert crab ID to a column. Join with metadata
...
module0_lipids <- module_df %>%
  ...
  filter(., Module == 0) %>%
  dplyr::select(., Lipid) %>% 
  left_join(., (rawLipidomicsData %>% dplyr::rename(Lipid = identifier)), by = "Lipid") %>%
  right_join(inChiKeyNamePubmed, ., by = "INCHIKEY") %>%
