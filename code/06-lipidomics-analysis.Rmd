...
lipidomicsMetadata <- read.csv("../../data/metabolomics_metadata.csv", header = FALSE, strip.white = TRUE) %>% 
  ...
  mutate(., day = gsub(x = day, pattern = " ", replacement = ""))  %>% #Import metadata. Do not include any header information. Remove "V" from column names. Transform so crab.ID, treatment, and day are all columns. Set the first row in the dataframe as column names, then remove that row.
  left_join(x = .,
            y = (read.csv("../../data/time-to-right.csv", header = TRUE, strip.white = TRUE) %>%
                   filter(., is.na(sampling.ID) == FALSE) %>%
                   mutate(crab.ID = as.factor(sampling.ID)) %>%
                   dplyr::select(crab.ID, sex)),
            by = "crab.ID") #Add sex information from TTR dataset to metadata
...
transLipidData[, c(6:2408)] <- transLipidData[, c(6:2408)] %>% mutate_if(is.character, as.numeric) #Convert data columns to numeric
...
head(lipidIdentifiers) #Confirm dataframe changes
```
...
```{r}
dissim.lipid <- vegdist(scale((transLipidData[c(6:2408)] %>%
                                 log() %>%
                                 mutate_if(is.numeric, list(~na_if(., -Inf))) %>%
                                 replace(is.na(.), 0))), "euclidean") #Create euclidean dissimilarity matrix to use for perMANOVA and PERMDISP
```

```{r}
permanova.lipid.sex <- adonis2(dissim.lipid ~ temperature + day + temperature:day +
                                 sex + sex:temperature, 
                               data = transLipidData, by = "term") #Conduct global PERMANOVA to assess significance of trends in PCA. Scale lipidomics data and assess influence of treatment, day, and treatment x day on metabolite profiles
permanova.lipid.sex #Significant influence temperature only.
```

```{r}
broom::tidy(permanova.lipid.sex) %>%
  write.csv(., "PCA/all-data-PERMANOVA-results.csv", quote = FALSE) #Save PERMANOVA output
```

I will need to use a PERMDISP test to determine if significant differences in temperature are due to centroid or variance differences.
...
```{r}
broom::tidy(anova(disp.lipid.temp)) %>%
  mutate(., var = "temperature") %>%
  write.csv("PCA/all-data-PERMDISP-results.csv", quote = FALSE, row.names = FALSE) #Clean output and save
```

## PCA and perMANOVA for known lipids

```{r}
knownTransLipidData <- transLipidData %>%
  dplyr::select(1:5, which(colnames(transLipidData) %in% lipidIdentifiers$lipid[1:418])) #Take transposed lipid data and select columns associated with known lipids.
head(knownTransLipidData)
```

```{r}
knownTransLipidData %>% #Take list of known lipids
  dplyr::select(-c(temperature:sex)) %>% #Remove metadata columns
  column_to_rownames(., var = "crab.ID") %>% #Convert crab ID to rownames
  t(.) %>% as.data.frame(.) %>% #Transpose and convert to dataframe
  rownames_to_column(., var = "lipid") %>% #Convert rownames to column
  write.table(., "known-lipid-list.txt", sep = "\t", quote = FALSE) #Save as a txt file
```


```{r}
scaled.known.pca <- prcomp(knownTransLipidData[,-c(1:5)] %>%
                             log() %>%
                             mutate_if(is.numeric, list(~na_if(., -Inf))) %>%
                             replace(is.na(.), 0), 
                           scale = FALSE, center = TRUE) #Use prcomp to perform a centered and scaled PCA calculation. Replace all -Inf with NA, and then with 0 in the dataframe and log transform prior to performing the PCA calculation
summary(scaled.known.pca) #Standard deviations for each PC
```

```{r}
pcaPlot4 <- autoplot(scaled.known.pca, data = transLipidData, x = 1, y = 2,
                     size = 4, alpha = 0.8, colour = "treatment", shape = "day", 
                     frame = TRUE, frame.colour = "treatment", loadings = FALSE) +
  scale_color_manual(values = c(plotColors[2], plotColors[1], plotColors[3]),
                     name = "Temperature (ºC)") +
  ...
  theme_classic(base_size = 15) + theme(legend.position = "right",
                                        plot.background = element_blank()) #Use autoplot to plot the prcomp object. Color points and confidence ellipses by treatment, and have different shapes for each day. Do not include loadings. Manually define color, fill, and shapes.
pcaPlot4
ggsave("PCA/figures/known-lipids-PCA-day-treatment.pdf", height = 8.5, width = 11)
```

```{r}
dissim.lipidid.sex <- vegdist(scale((knownTransLipidData[-c(1:5)] %>%
                                       log() %>%
                                       mutate_if(is.numeric, list(~na_if(., -Inf))) %>%
                                       replace(is.na(.), 0))), "euclidean") #Create euclidean dissimilarity matrix to use for perMANOVA and PERMDISP
```
...
```{r}
permanova.lipidid <- adonis2(dissim.lipidid.sex ~ temperature*day +
                               sex + sex:temperature, 
                             data = knownTransLipidData, by = "term") #Conduct global PERMANOVA to assess significance of trends in PCA. Scale lipidomics data and assess influence of treatment, day, and treatment x day on metabolite profiles
permanova.lipidid #Significant influence temperature only
```

```{r}
broom::tidy(permanova.lipidid) %>%
  write.csv(., "PCA/known-lipids-PERMANOVA-results.csv", quote = FALSE) #Save PERMANOVA output
```

Day was not significant when considering all lipids, but is significant when considering known lipids. To be conservative, I will stick to only investigating temperature effects on lipids.

```{r}
disp.lipidid.temp <- betadisper(dissim.lipidid, group = as.factor(knownTransLipidData$treatment), type = 'centroid') #Run a beta dispersion model to assess if significant differences are due to differences in group centroid or variance
plot(disp.lipidid.temp) #Dispersion differences exist
boxplot(disp.lipidid.temp) #Show distance to group centroids, can clearly see dispersion differences between 30 and the other two
anova(disp.lipidid.temp) #Variance is different between groups. Temperature significance is due to centroid and dispersion differences!
```

```{r}
broom::tidy(anova(disp.lipidid.temp)) %>%
  mutate(., var = "temperature") %>%
  write.csv("PCA/known-lipids-PERMDISP-results.csv", quote = FALSE, row.names = FALSE) #Clean output and save
```
...
knownTransLipidData %>%
  ...
  dplyr::select(temperature, any_of(significantVIP_13v30$lipid)) %>%
...
enrichmentResults13v30 %>% filter(., FDR.q.value < 0.05) #There are 7 significantly enriched terms
```
...
LIONtoTerm13v30 <- read.csv("../06-lipidomics-analysis/metaboanalyst/LION-enrichment-report-13v30/LION-term-associations-13v30.csv") %>%
  ...
  slice(-1) %>%
  filter(LION.term == "LION:0012080" | LION.term == "LION:0000095" | LION.term == "LION:0000465" | LION.term == "LION:0000010" | LION.term == "LION:0012010" | LION.term == "LION:0000003") %>% 
...
```{r}
lipidsLIONtoTerm13v30 <- read.csv("../06-lipidomics-analysis/metaboanalyst/LION-enrichment-report-13v30/LION-lipid-associations-13v30.csv", strip.white = TRUE, na.strings = "") %>% #Import file that associates individual lipids with LION terms. Remove any white text and convert blanks to NAs
  slice(-1) %>% #Remove first row
  mutate(LION.term = substr(x = LION.term, start = 9, stop = 1000000L)) %>% #Remove extra characters from the lion term
  rename(lipid = LION.term) %>% #Rename LION.term to lipid
  mutate(lipid = str_trim(lipid, side = "both")) %>% #Remove white space from lipid name
  unique(.) %>% #Keep only unique entries
  remove_rownames() %>% #Remove rownames
  column_to_rownames(., var = "lipid") %>% #Convert lipid column to rownames
  dplyr::select_if(colnames(.) %in% LIONtoTerm13v30$LION.term) %>% #Retain columns if they are enriched LION terms
  rownames_to_column(., var = "lipid") %>% #Convert row names to lipid column
  right_join(x = ., 
             y = (t.test_13v30 %>%
                    ungroup(.) %>%
                    filter(., p.adj < 0.05) %>%
                    dplyr::select(lipid)), 
             by = "lipid") %>% #Use right_join to only retain rows with lipids that are VIP for the contrast of interest
  column_to_rownames(var = "lipid") %>% #Convert column to rowname
  filter(!if_all(everything(), is.na)) #Remove rows where all entries are NA
head(lipidsLIONtoTerm13v30) #Confirm dataframe creation
```

```{r}
#Make individual dataframes that list the lipids associated with each LION term
VIPlist.LION.0000003 <- lipidsLIONtoTerm13v30 %>% #Take dataframe that matches lipid names with LION terms
  dplyr::select(LION.0000003) %>% #Select LION term
  filter(., is.na(LION.0000003) == FALSE) %>% #Remove rows with NAs
  rownames_to_column("lipid") #Convert rowname to column

VIPlist.LION.0000010 <- lipidsLIONtoTerm13v30 %>% #Take dataframe that matches lipid names with LION terms
  dplyr::select(LION.0000010) %>% #Select LION term
  filter(., is.na(LION.0000010) == FALSE) %>% #Remove rows with NAs
  rownames_to_column("lipid") #Convert rowname to column

VIPlist.LION.0000095 <- lipidsLIONtoTerm13v30 %>% #Take dataframe that matches lipid names with LION terms
  dplyr::select(LION.0000095) %>% #Select LION term
  filter(., is.na(LION.0000095) == FALSE) %>% #Remove rows with NAs
  rownames_to_column("lipid") #Convert rowname to column

VIPlist.LION.0000465 <- lipidsLIONtoTerm13v30 %>% #Take dataframe that matches lipid names with LION terms
  dplyr::select(LION.0000465) %>% #Select LION term
  filter(., is.na(LION.0000465) == FALSE) %>% #Remove rows with NAs
  rownames_to_column("lipid") #Convert rowname to column

VIPlist.LION.0012010 <- lipidsLIONtoTerm13v30 %>% #Take dataframe that matches lipid names with LION terms
  dplyr::select(LION.0012010) %>% #Select LION term
  filter(., is.na(LION.0012010) == FALSE) %>% #Remove rows with NAs
  rownames_to_column("lipid") #Convert rowname to column

VIPlist.LION.0012080 <- lipidsLIONtoTerm13v30 %>% #Take dataframe that matches lipid names with LION terms
  dplyr::select(LION.0012080) %>% #Select LION term
  filter(., is.na(LION.0012080) == FALSE) %>% #Remove rows with NAs
  rownames_to_column("lipid") #Convert rowname to column
```

```{r}
#Create dataframes for plotting for each LION term
percDiff.LION.0000003 <- knownTransLipidData %>% #Take known lipid abundance data
  dplyr::select(temperature, any_of(VIPlist.LION.0000003$lipid)) %>% #Retain columns with VIP lipids
  mutate(., temperature = str_trim(temperature, side = "both"),
         temperature = case_when(temperature == "5" ~ "05C",
                                 temperature == "13" ~ "13C",
                                 temperature == "30" ~ "30C")) %>% #Remove extra spaces on each side of temperature information, convert temperature information to characters
  group_by(temperature) %>% #Group by temperature
  summarize_all(funs(mean(., na.rm = TRUE))) %>% #Calculate averages for each lipid
  column_to_rownames(., var = "temperature") %>% #Convert column to rownames
  t(.) %>% as.data.frame(.) %>% #Transpose and covert to a dataframe
  mutate(., percDiff05 = ((`05C` - `13C`)/(`13C`))*100,
         percDiff30 = ((`30C` - `13C`)/(`13C`))*100) %>% #Calculate percent difference from 13
  dplyr::select(percDiff05:percDiff30) %>% #Retain difference columns
  rownames_to_column(., var = "lipid") %>% #Convert rownames to column
  pivot_longer(., cols = 2:ncol(.), names_to = "treatment", values_to = "percDiff") %>% #Pivot longer, moving percDiff column names to treatment and values to percDiff
  mutate(., LION.term = "LION.0000003") #Add column with LION term ID

percDiff.LION.0000010 <- knownTransLipidData %>% #Take known lipid abundance data
  dplyr::select(temperature, any_of(VIPlist.LION.0000010$lipid)) %>% #Retain columns with VIP lipids
  mutate(., temperature = str_trim(temperature, side = "both"),
         temperature = case_when(temperature == "5" ~ "05C",
                                 temperature == "13" ~ "13C",
                                 temperature == "30" ~ "30C")) %>% #Remove extra spaces on each side of temperature information, convert temperature information to characters
  ...
  rownames_to_column(., var = "lipid") %>% #Convert rownames to column
  pivot_longer(., cols = 2:ncol(.), names_to = "treatment", values_to = "percDiff") %>% #Pivot longer, moving percDiff column names to treatment and values to percDiff
  mutate(., LION.term = "LION.0000010") #Add column with LION term ID

percDiff.LION.0000095 <- knownTransLipidData %>% #Take known lipid abundance data
  dplyr::select(temperature, any_of(VIPlist.LION.0000095$lipid)) %>% #Retain columns with VIP lipids
  mutate(., temperature = str_trim(temperature, side = "both"),
         temperature = case_when(temperature == "5" ~ "05C",
                                 temperature == "13" ~ "13C",
                                 temperature == "30" ~ "30C")) %>% #Remove extra spaces on each side of temperature information, convert temperature information to characters
  ...
  rownames_to_column(., var = "lipid") %>% #Convert rownames to column
  pivot_longer(., cols = 2:ncol(.), names_to = "treatment", values_to = "percDiff") %>% #Pivot longer, moving percDiff column names to treatment and values to percDiff
  mutate(., LION.term = "LION.0000095") #Add column with LION term ID

percDiff.LION.0000465 <- knownTransLipidData %>% #Take known lipid abundance data
  dplyr::select(temperature, any_of(VIPlist.LION.0000465$lipid)) %>% #Retain columns with VIP lipids
  mutate(., temperature = str_trim(temperature, side = "both"),
         temperature = case_when(temperature == "5" ~ "05C",
                                 temperature == "13" ~ "13C",
                                 temperature == "30" ~ "30C")) %>% #Remove extra spaces on each side of temperature information, convert temperature information to characters
  ...
  rownames_to_column(., var = "lipid") %>% #Convert rownames to column
  pivot_longer(., cols = 2:ncol(.), names_to = "treatment", values_to = "percDiff") %>% #Pivot longer, moving percDiff column names to treatment and values to percDiff
  mutate(., LION.term = "LION.0000465") #Add column with LION term ID

percDiff.LION.0012010 <- knownTransLipidData %>% #Take known lipid abundance data
  dplyr::select(temperature, any_of(VIPlist.LION.0012010$lipid)) %>% #Retain columns with VIP lipids
  mutate(., temperature = str_trim(temperature, side = "both"),
         temperature = case_when(temperature == "5" ~ "05C",
                                 temperature == "13" ~ "13C",
                                 temperature == "30" ~ "30C")) %>% #Remove extra spaces on each side of temperature information, convert temperature information to characters
  ...
  rownames_to_column(., var = "lipid") %>% #Convert rownames to column
  pivot_longer(., cols = 2:ncol(.), names_to = "treatment", values_to = "percDiff") %>% #Pivot longer, moving percDiff column names to treatment and values to percDiff
  mutate(., LION.term = "LION.0012010") #Add column with LION term ID

percDiff.LION.0012080 <- knownTransLipidData %>% #Take known lipid abundance data
  dplyr::select(temperature, any_of(VIPlist.LION.0012080$lipid)) %>% #Retain columns with VIP lipids
  mutate(., temperature = str_trim(temperature, side = "both"),
         temperature = case_when(temperature == "5" ~ "05C",
                                 temperature == "13" ~ "13C",
                                 temperature == "30" ~ "30C")) %>% #Remove extra spaces on each side of temperature information, convert temperature information to characters
  ...
  rownames_to_column(., var = "lipid") %>% #Convert rownames to column
  pivot_longer(., cols = 2:ncol(.), names_to = "treatment", values_to = "percDiff") %>% #Pivot longer, moving percDiff column names to treatment and values to percDiff
  mutate(., LION.term = "LION.0012080") #Add column with LION term ID
```

```{r}
percDiff.13v30.enriched <- rbind(percDiff.LION.0000003,
                                 percDiff.LION.0000010,
                                 percDiff.LION.0000095,
                                 percDiff.LION.0000465,
                                 percDiff.LION.0012010,
                                 percDiff.LION.0012080) #Use rbind to combine dataframes into one
tail(percDiff.13v30.enriched) #Confirm dataframe creation
```

```{r}
percDiff.13v30.enriched %>%
  group_by(LION.term) %>%
  summarize(count = n()) #Count the number of lipids in each LION term
```

```{r}
percDiff.13v30.facetLabels <- c("LION.0000003" = "LION.0000003 (n = 64)", 
                                "LION.0000010" = "LION.0000010 (n = 48)", 
                                "LION.0000095" = "LION.0000095 (n = 70)", 
                                "LION.0000465" = "LION.0000465 (n = 48)", 
                                "LION.0012010" = "LION.0012010 (n = 70)", 
                                "LION.0012080" = "LION.0012080 (n = 66)") #Create facet labels
```


```{r}
percDiff.13v30.enriched %>% #Dataframe with percent differences
  ggplot(., aes(x = treatment, y = percDiff, color = treatment, fill = treatment)) +
  geom_point(pch = 21, position = position_dodge(width = 0.5), alpha = 0.3) +
  geom_hline(yintercept = 0, color = "grey70", linetype = 2) + #Add zero line
  facet_wrap(~LION.term, labeller = labeller(LION.term = percDiff.13v30.facetLabels)) +
  ylab("Difference from 13ºC (%)") + #y axis label
  scale_fill_manual(name = "Temperature (ºC)", 
...
ggsave("PLSDA/figures/percent-diff-enriched-pathways-13v30.pdf", height = 8.5, width = 11) #Save figure
```
...
```{r}
lipidsLIONtoTerm13v5 <- read.csv("../06-lipidomics-analysis/metaboanalyst/LION-enrichment-report-13v5/LION-lipid-associations-13v5.csv", strip.white = TRUE, na.strings = "") %>% #Import file that associates individual lipids with LION terms. Remove any white text and convert blanks to NAs
  slice(-1) %>% #Remove first row
  mutate(LION.term = substr(x = LION.term, start = 9, stop = 1000000L)) %>% #Remove extra characters from the lion term
  rename(lipid = LION.term) %>% #Rename LION.term to lipid
  mutate(lipid = str_trim(lipid, side = "both")) %>% #Remove white space from lipid name
  ...
  remove_rownames() %>% #Remove rownames
  column_to_rownames(., var = "lipid") %>% #Convert lipid column to rownames
  dplyr::select_if(colnames(.) %in% LIONtoTerm13v5$LION.term) %>% #Retain columns if they are enriched LION terms
  rownames_to_column(., var = "lipid") %>% #Convert row names to lipid column
  right_join(x = ., 
             y = (t.test_13v5 %>%
                    ungroup(.) %>%
  ...
  column_to_rownames(var = "lipid") %>% #Convert column to rowname
  filter(!if_all(everything(), is.na)) #Remove rows where all entries are NA
head(lipidsLIONtoTerm13v5) #Confirm dataframe creation
```

```{r}
#Make individual dataframes that list the lipids associated with each LION term
VIPlist.LION.0000003 <- lipidsLIONtoTerm13v5 %>% #Take dataframe that matches lipid names with LION terms
  dplyr::select(LION.0000003) %>% #Select LION term
  filter(., is.na(LION.0000003) == FALSE) %>% #Remove rows with NAs
  rownames_to_column("lipid") #Convert rowname to column

VIPlist.LION.0000011 <- lipidsLIONtoTerm13v5 %>% #Take dataframe that matches lipid names with LION terms
  dplyr::select(LION.0000011) %>% #Select LION term
  filter(., is.na(LION.0000011) == FALSE) %>% #Remove rows with NAs
  rownames_to_column("lipid") #Convert rowname to column

VIPlist.LION.0012080 <- lipidsLIONtoTerm13v5 %>% #Take dataframe that matches lipid names with LION terms
  dplyr::select(LION.0012080) %>% #Select LION term
  filter(., is.na(LION.0012080) == FALSE) %>% #Remove rows with NAs
  rownames_to_column("lipid") #Convert rowname to column

VIPlist.LION.0012081 <- lipidsLIONtoTerm13v5 %>% #Take dataframe that matches lipid names with LION terms
  dplyr::select(LION.0012081) %>% #Select LION term
  filter(., is.na(LION.0012081) == FALSE) %>% #Remove rows with NAs
  rownames_to_column("lipid") #Convert rowname to column
```
...
percDiff.LION.0000011 <- knownTransLipidData %>% #Take known lipid abundance data
  dplyr::select(temperature, any_of(VIPlist.LION.0000011$lipid)) %>% #Retain columns with VIP lipids
  mutate(., temperature = str_trim(temperature, side = "both"),
         temperature = case_when(temperature == "5" ~ "05C",
                                 temperature == "13" ~ "13C",
                                 temperature == "30" ~ "30C")) %>% #Remove extra spaces on each side of temperature information, convert temperature information to characters
  ...
  rownames_to_column(., var = "lipid") %>% #Convert rownames to column
  pivot_longer(., cols = 2:ncol(.), names_to = "treatment", values_to = "percDiff") %>% #Pivot longer, moving percDiff column names to treatment and values to percDiff
  mutate(., LION.term = "LION.0000011") #Add column with LION term ID
...
percDiff.LION.0012081 <- knownTransLipidData %>% #Take known lipid abundance data
  dplyr::select(temperature, any_of(VIPlist.LION.0012081$lipid)) %>% #Retain columns with VIP lipids
  mutate(., temperature = str_trim(temperature, side = "both"),
         temperature = case_when(temperature == "5" ~ "05C",
                                 temperature == "13" ~ "13C",
                                 temperature == "30" ~ "30C")) %>% #Remove extra spaces on each side of temperature information, convert temperature information to characters
  ...
  rownames_to_column(., var = "lipid") %>% #Convert rownames to column
  pivot_longer(., cols = 2:ncol(.), names_to = "treatment", values_to = "percDiff") %>% #Pivot longer, moving percDiff column names to treatment and values to percDiff
  mutate(., LION.term = "LION.0012081") #Add column with LION term ID
```

```{r}
percDiff.13v5.enriched <- rbind(percDiff.LION.0000003,
                                percDiff.LION.0000011,
                                percDiff.LION.0012080,
                                percDiff.LION.0012081) #Use rbind to combine dataframes into one
tail(percDiff.13v5.enriched) #Confirm dataframe creation
```

```{r}
percDiff.13v5.enriched %>%
  group_by(LION.term) %>%
  summarize(count = n()) #Count the number of lipids in each LION term
```

```{r}
percDiff.13v5.facetLabels <- c("LION.0000003" = "LION.0000003 (n = 38)", 
                               "LION.0000011" = "LION.0000011 (n = 12)", 
                               "LION.0012080" = "LION.0012080 (n = 40)", 
                               "LION.0012081" = "LION.0012081 (n = 12)") #Create facet labels
```

```{r}
percDiff.13v5.enriched %>% #Dataframe with percent differences
  ggplot(., aes(x = treatment, y = percDiff, color = treatment, fill = treatment)) +
  geom_point(pch = 21, position = position_dodge(width = 0.5), alpha = 0.3) +
  geom_hline(yintercept = 0, color = "grey70", linetype = 2) + #Add zero line
  facet_wrap(~LION.term, labeller = labeller(LION.term = percDiff.13v5.facetLabels)) +
  ylab("Difference from 13ºC (%)") + #y axis label
  scale_fill_manual(name = "Temperature (ºC)", 
                    values = c("#2171B5", "#CB181D"),
                    labels = c("5ºC", "30ºC")) +
  ...
  theme_classic(base_size = 15) + theme(axis.text.x = element_blank(), 
                                        ...
                                        legend.text = element_text(color = "black", size = 12), 
                                        strip.text.x = element_text(size = 10, color = "black", face="bold"))
ggsave("PLSDA/figures/percent-diff-enriched-pathways-13v5.pdf", height = 8.5, width = 11) #Save figure
```

##### Multipanel figure

- make a new dataframe to add a label
- use geom_text to add label

```{r}
percDiff.13v30.enriched %>% #Percent difference dataframe
  filter(., treatment == "percDiff30") %>% #Filter for 13 v 30 information
  group_by(LION.term) %>% #Group by LION term
  summarize(., high30 = sum(percDiff > 0), #Number of lipids with higher abundance at 30
            high13 = sum(percDiff < 0)) #Number of lipids with lower abundance at 13
```

```{r}
percDiff.13v30.high_textMultipanel <- data.frame(label = c("9 higher at 30ºC",
                                                      "8 higher at 30ºC",
                                                      "12 higher at 30ºC",
                                                      "8 higher at 30ºC",
                                                      "12 higher at 30ºC",
                                                      "9 higher at 30ºC"), #Label for each facet
                                            LION.term = LIONtoTerm13v30$LION.term, #Assign label to specific facet
                                            x = rep(0.45, times = 6), #x location
                                            y = rep(220, times = 6)) #y location
```

```{r}
percDiff.13v30.low_textMultipanel <- data.frame(label = c("23 higher at 13ºC",
                                                      "16 higher at 13ºC",
                                                      "23 higher at 13ºC",
                                                      "16 higher at 13ºC",
                                                      "23 higher at 13ºC",
                                                      "24 higher at 13ºC"), #Label for each facet
                                            LION.term = LIONtoTerm13v30$LION.term, #Assign label to specific facet
                                            x = rep(0.45, times = 6), #x location
                                            y = rep(-70, times = 6)) #y location
```

```{r}
percDiff.13v30.figure <- percDiff.13v30.enriched %>% #Dataframe with percent differences
  ggplot(., aes(x = treatment, y = percDiff, color = treatment, fill = treatment)) +
  geom_point(pch = 21, position = position_dodge(width = 0.5), size = 2.5, alpha = 0.3) +
  geom_hline(yintercept = 0, color = "grey70", linetype = 2) + #Add zero line
  # annotate("text", label = "higher at 30ºC", x = 0.45, y = 220, size = 4, color = plotColors[1], hjust = 0) +
  geom_text(data = percDiff.13v30.high_textMultipanel, mapping = aes(x = x, y = y, label = label), inherit.aes = FALSE, size = 4, color = plotColors[1], hjust = 0) + #Add labels to each individual facet
  # annotate("text", label = "higher at 13ºC", x = 0.45, y = -70, size = 4, color = plotColors[2], hjust = 0) +
    geom_text(data = percDiff.13v30.low_textMultipanel, mapping = aes(x = x, y = y, label = label), inherit.aes = FALSE, size = 4, color = plotColors[2], hjust = 0) + #Add labels to each individual facet
  facet_wrap(~LION.term, labeller = labeller(LION.term = percDiff.13v30.facetLabels)) +
  ylab("") + #y axis label
  scale_fill_manual(name = "Temperature (ºC)", 
  ...
  scale_colour_manual(name = "Temperature (ºC)", 
                      ...
                      labels = c("5ºC", "30ºC")) +
  xlab("") + #x axis label
  ggtitle("A. Pairwise VIP lipids for 13ºC vs. 30ºC") +
  theme_classic(base_size = 15) + theme(axis.text.x = element_blank(), 
                                        axis.ticks.x = element_blank(),
                                        legend.position = "bottom", 
                                        legend.text = element_text(color = "black", size = 12), 
                                        strip.text.x = element_text(size = 10, color = "black", face="bold"))
percDiff.13v30.figure
```

```{r}
percDiff.13v5.enriched %>% #Percent difference dataframe
  filter(., treatment == "percDiff05") %>% #Filter for 13 v 5 information
  group_by(LION.term) %>% #Group by LION term
  summarize(., high05 = sum(percDiff > 0), #Number of lipids with higher abundance at 5
            high13 = sum(percDiff < 0)) #Number of lipids with lower abundance at 13
```

```{r}
percDiff.13v5.high_textMultipanel <- data.frame(label = c("1 higher at 5ºC",
                                                      "1 higher at 5ºC",
                                                      "1 higher at 5ºC",
                                                      "1 higher at 5ºC"), #Label for each facet
                                            LION.term = LIONtoTerm13v5$LION.term, #Assign label to specific facet
                                            x = rep(0.45, times = 4), #x location
                                            y = rep(220, times = 4)) #y location
```

```{r}
percDiff.13v5.low_textMultipanel <- data.frame(label = c("18 higher at 13ºC",
                                                      "5 higher at 13ºC",
                                                      "19 higher at 13ºC",
                                                      "5 higher at 13ºC"), #Label for each facet
                                            LION.term = LIONtoTerm13v5$LION.term, #Assign label to specific facet
                                            x = rep(0.45, times = 4), #x location
                                            y = rep(-70, times = 4)) #y location
```

```{r}
percDiff.13v5.figure <- percDiff.13v5.enriched %>% #Dataframe with percent differences
  ggplot(., aes(x = treatment, y = percDiff, color = treatment, fill = treatment)) +
  geom_point(pch = 21, position = position_dodge(width = 0.5), size = 2.5, alpha = 0.3) +
  geom_hline(yintercept = 0, color = plotColors[2], linetype = 2) + #Add zero line
  # annotate("text", label = "higher at 30ºC", x = 0.45, y = 220, size = 4, color = plotColors[1], hjust = 0) +
  geom_text(data = percDiff.13v5.high_textMultipanel, mapping = aes(x = x, y = y, label = label), inherit.aes = FALSE, size = 4, color = plotColors[3], hjust = 0) + #Add labels to each individual facet
  # annotate("text", label = "higher at 13ºC", x = 0.45, y = -70, size = 4, color = plotColors[2], hjust = 0) +
    geom_text(data = percDiff.13v5.low_textMultipanel, mapping = aes(x = x, y = y, label = label), inherit.aes = FALSE, size = 4, color = plotColors[2], hjust = 0) + #Add labels to each individual facet
  facet_wrap(~LION.term, labeller = labeller(LION.term = percDiff.13v5.facetLabels)) +
  ylab("") + #y axis label
  scale_fill_manual(name = "Temperature (ºC)", 
                    values = c(plotColors[3], plotColors[1]),
  ...
  scale_colour_manual(name = "Temperature (ºC)", 
                      ...
                      labels = c("5ºC", "30ºC")) +
  xlab("") + #x axis label
  ggtitle("B. Pairwise VIP lipids for 13ºC vs. 5ºC") +
  theme_classic(base_size = 15) + theme(axis.text.x = element_blank(), 
                                        axis.ticks.x = element_blank(),
                                        legend.position = "bottom", 
                                        legend.text = element_text(color = "black", size = 12), 
                                        strip.text.x = element_text(size = 10, color = "black", face="bold"))
percDiff.13v5.figure
```

```{r}
enrichmentFigure <-(percDiff.13v30.figure /
                      percDiff.13v5.figure) + #Combine figures vertically
...
y_axis_label <- textGrob("Difference from 13ºC (%)", rot = 90, gp = gpar(fontsize = 15))
enrichmentFigureWithLabel <- wrap_elements(y_axis_label) + enrichmentFigure + plot_layout(widths = c(1, 40))

ggsave("PLSDA/figures/percent-diff-enriched-pathways-multipanel.pdf", width = 8.5, height = 11, device = cairo_pdf) #Save figure
```
...
I am going to use ASCA to classify trends in lipid abundance over temperature. Similar to the WCNA, this will provide me groups of compounds, but in a more statistically rigorous manner.
...
```{r}
lipid.ASCA <- rowwiseLipidData %>%
  rownames_to_column(., var = "crab.ID") %>%
  pivot_longer(., cols = !crab.ID, names_to = "variable", values_to = "value") %>%
  left_join(lipidomicsMetadata, ., by = "crab.ID") %>%
  dplyr::select(-c(day, treatment_day)) %>%
  mutate(., variable = str_trim(variable, side = "both"),
         treatment = case_when(treatment == "5" ~ "05",
                               treatment != "5" ~ treatment)) %>%
  mutate(., treatment = as.factor(treatment)) %>%
  unique(.) %>%
  group_by(crab.ID, treatment, variable) %>%
  summarise(value = sum(value, na.rm = TRUE)) #Take data that has already been log-transformed and corrected for 0s. Pivot data longer for ASCA. Compound information should be "variable." Add metadata to dataframe with all compound information. Remove day, change temperature to 05, and refactor. Filter for unique rows and summarise values since there are some instances of duplication (sequencer did not catch all components)
head(lipid.ASCA) #Confirm changes
```

## Run model

```{r}
lipid.ASCAmodel <- ALASCA(df = lipid.ASCA,
                          formula = value ~ treatment + (1|crab.ID),
                          n_validation_runs = 100,
                          validate = TRUE,
                          reduce_dimensions = TRUE) #Run ASCA to identify temperature effects on compound abundance
```
...
plot(lipid.ASCAmodel, component = 1:3, type = "scree") #Scree plot of variance explained by each PC. There were 3 components identified, with the third component explaining very little variance
# dev.off()
```

### Component 1

This component shows lipids with lower absolute abundance at 5ºC and much lower absolute abundance at 30ºC

```{r}
plot(lipid.ASCAmodel, effect = 1, component = 1, type = 'effect', 
     n_limit = 5, 
     ribbon = TRUE,
...
```{r}
plot(lipid.ASCAmodel, effect = 1, component = 1, type = 'prediction', 
     n_limit = 10, 
     ribbon = TRUE,
     x_angle = 0, x_h_just= 0.5,
     flip_axis = TRUE,
...
```{r}
loadingsPC1 <- get_loadings(lipid.ASCAmodel, component = 1, effect = 1) #Get loadings for the top 10 positively and negatively loaded compounds on the PC
head(loadingsPC1)
```
...
loadingsPC1.annot <- loadingsPC1[[1]] %>%
  ...
  left_join(x = .,
            y = read.csv("../06-lipidomics-analysis/metaboanalyst/LION-enrichment-report-13v30/LION-lipid-associations-13v30.csv", strip.white = TRUE) %>%
              ...
              filter(., lipid %in% (loadingsPC1[[1]]$covars %>%
                                      str_trim(side = "both"))) %>%
              unique(.) %>%
              remove_rownames() %>%
...
loadingsPC1[[1]] %>%
  ...
  left_join(x = .,
            y = (rawLipidomicsData %>%
                   mutate(lipid = str_trim(lipid, side = "both"))),
            by = "lipid") %>%
  dplyr::select(., PC, lipid, loading, cum_var, INCHIKEY) %>%
  unique(.) %>%
  nrow(.) #84 compounds explain 50% of the cumulative variance in this PC
```
...
loadingsPC1[[1]] %>%
  ...
  dplyr::select(., PC, lipid, loading, cum_var, INCHIKEY) %>%
  unique(.) %>%
  write.table("ASCA/PC1_cumvar50_lipids.txt", sep = "\t", quote = FALSE, row.names = FALSE) #Save list of metabolites to use with LION/web
```

```{r}
enrichmentResultsPC1 <- read.csv("ASCA/LION-enrichment-report-PC1/LION-enrichment-results-PC1.csv") %>%
  dplyr::rename(Description = "Discription") %>%
  dplyr::rename(Observed = "Significant") %>%
  mutate(., Significant = case_when(FDR.q.value < 0.05 ~ "Yes",
                                    FDR.q.value >= 0.05 ~ "No")) %>%
  mutate(., "-log(FDR)" = -log(FDR.q.value)) %>%
  arrange(desc("-log(FDR)")) #Import enrichment results. Rename description and observed column and create new column
head(enrichmentResultsPC1) #Look at results
enrichmentResultsPC1 %>% filter(., FDR.q.value < 0.05) %>% nrow() #There are no significantly enriched terms
```

### Component 2

This component shows lipids with lower absolute abundance at 30ºC and much lower absolute abundance at 5ºC

```{r}
plot(lipid.ASCAmodel, effect = 1, component = 2, type = 'effect', 
     n_limit = 5, 
     ribbon = TRUE,
     x_angle = 0, x_h_just= 0.5,
     flip_axis = TRUE,
...
```{r}
plot(lipid.ASCAmodel, effect = 1, component = 2, type = 'prediction', 
     n_limit = 10, 
     ribbon = TRUE,
     x_angle = 0, x_h_just= 0.5,
     flip_axis = TRUE,
...
```{r}
loadingsPC2 <- get_loadings(lipid.ASCAmodel, component = 2, effect = 1) #Get loadings for the top 10 positively and negatively loaded compounds on the PC
head(loadingsPC2)
```
...
loadingsPC2[[1]] %>%
  ...
  dplyr::select(., PC, lipid, loading, cum_var, INCHIKEY) %>%
  unique(.) %>%
  nrow(.) #63 compounds explain 50% of the cumulative variance in this PC
```
...
loadingsPC2[[1]] %>%
  ...
  dplyr::select(., PC, lipid, loading, cum_var, INCHIKEY) %>%
  unique(.) %>%
  write.table("ASCA/PC2_cumvar50_lipids.txt", sep = "\t", quote = FALSE, row.names = FALSE) #Save list of metabolites to use with LION/web
```

```{r}
enrichmentResultsPC2 <- read.csv("ASCA/LION-enrichment-report-PC2/LION-enrichment-results-PC2.csv") %>%
  dplyr::rename(Description = "Discription") %>%
  dplyr::rename(Observed = "Significant") %>%
  mutate(., Significant = case_when(FDR.q.value < 0.05 ~ "Yes",
                                    FDR.q.value >= 0.05 ~ "No")) %>%
  mutate(., "-log(FDR)" = -log(FDR.q.value)) %>%
  arrange(desc("-log(FDR)")) #Import enrichment results. Rename description and observed column and create new column
head(enrichmentResultsPC2) #Look at results
enrichmentResultsPC2 %>% filter(., FDR.q.value < 0.05) %>% nrow() #There are no significantly enriched terms
```

### Component 3

This component shows lipids with higher absolute abundance at 5ºC, but it also explains 0% of the variance. I'll focus my analyses on PCs 1 and 2, since those explain >10% of the variance (same cutoff used with metabolomics data).

```{r}
plot(lipid.ASCAmodel, effect = 1, component = 3, type = 'effect', 
     n_limit = 5, 
     ribbon = TRUE,
     x_angle = 0, x_h_just= 0.5,
     flip_axis = TRUE,
...
```{r}
plot(lipid.ASCAmodel, effect = 1, component = 3, type = 'prediction', 
     n_limit = 10, 
     ribbon = TRUE,
     x_angle = 0, x_h_just= 0.5,
     flip_axis = TRUE,
...
```{r}
loadingsPC3 <- get_loadings(lipid.ASCAmodel, component = 3, effect = 1, n_limit = 10) #Get loadings for the top 10 positively and negatively loaded compounds on the PC
head(loadingsPC3)
```
...
loadingsPC3.annot <- loadingsPC3[[1]] %>%
  ...
  left_join(x = .,
            y = read.csv("../06-lipidomics-analysis/metaboanalyst/LION-enrichment-report-13v30/LION-lipid-associations-13v30.csv", strip.white = TRUE) %>%
              ...
              rename(lipid = LION.term) %>%
              mutate(lipid = str_trim(lipid, side = "both")) %>%
              filter(., lipid %in% (loadingsPC3[[1]]$covars %>%
                                      str_trim(side = "both"))) %>%
              unique(.) %>%
              remove_rownames() %>%
              column_to_rownames(., var = "lipid") %>%
...
```{r}
plot(lipid.ASCAmodel, effect = 1, component = c(1,2), type = 'effect', 
     n_limit = 5, 
     ribbon = TRUE,
     x_angle = 0, x_h_just= 0.5,
     flip_axis = TRUE,
...
I want to create a multipanel figure with all of the prediction plots

```{r}
predictionPC1 <- plot(lipid.ASCAmodel, effect = 1, component = 1, type = 'prediction', 
                      n_limit = 10, 
                      ribbon = TRUE,
                      x_angle = 0, x_h_just= 0.5,
                      flip_axis = TRUE,
  ...
  ggtitle("A. PC 1") +
  theme_classic() + theme(strip.text.x = element_text(size = 6, color = "black"))

predictionPC2 <- plot(lipid.ASCAmodel, effect = 1, component = 2, type = 'prediction', 
                      n_limit = 10, 
                      ribbon = TRUE,
                      x_angle = 0, x_h_just= 0.5,
                      flip_axis = TRUE,
...
predictionPC1 / predictionPC2 + plot_layout(guides = 'collect')
ggsave("../../supplementary-material/lipid-ASCA-prediction-plot.pdf", width = 8.5, height = 11)
```
