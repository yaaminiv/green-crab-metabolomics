...
transLipidData <- rawLipidomicsData %>%
  ...
  left_join(lipidomicsMetadata, ., by = "crab.ID") #Remove MS information, convert lipid names to rownames, and transform dataframe. Convert rownames to crab.ID column
...
```{r}
X <- log(transLipidData[c(5:617)]) #Assign known metabolite data to the X dataframe after log transformation
Y <- as.factor(transLipidData$treatment_day) #Assign treatment data to the Y dataframe
```
...
plotIndiv(treatmentday.known.plsda, comp = c(1,2), ind.names = FALSE,
          ...
          ellipse = TRUE, star = TRUE, centroid = FALSE, 
          legend = TRUE, legend.title = "Temperature (ºC) x Day", 
          title = "PLS-DA by Temperature and Day", X.label = "Component 1: 29% Explained Variance", size.xlabel = 15, Y.label = "Component 2: 15% Explained Variance", size.ylabel = 15, size.axis = 13,
          style = "ggplot2") #Visualize first two components of PLS-DA with no individual sample names. Alter point size, color, shape, and line width. Include 95% confidence ellipses and use star format to visualize sample distance from centroids. Include a legend and specify the legend title. Specify the plot title and text label sizes. Use ggplot style.
#dev.off()
```
...
plotIndiv(treatmentday.known.plsda, comp = c(1,3), ind.names = FALSE,
          ...
          ellipse = TRUE, star = TRUE, centroid = FALSE, 
          legend = TRUE, legend.title = "Temperature (ºC) x Day", 
          title = "PLS-DA by Temperature and Day", X.label = "Component 1: 29% Explained Variance", size.xlabel = 15, Y.label = "Component 3: 8% Explained Variance", size.ylabel = 15, size.axis = 13,
          style = "ggplot2") #Visualize first two components of PLS-DA with no individual sample names. Alter point size, color, shape, and line width. Include 95% confidence ellipses and use star format to visualize sample distance from centroids. Include a legend and specify the legend title. Specify the plot title and text label sizes. Use ggplot style.
#dev.off()
```
...
treatmentday.known.VIP %>%
  filter(VIP >= 1) %>%
  nrow(.) #73 lipids have VIP > 1
274/612 * 100 #44.8% lipids VIP > 1
```
...
VIP_select_13v30_day3 <- cbind(clean_13v30_day3[, 1:4], 
  ...
  pivot_longer(., cols = 5:269, values_to = "log_norm_counts", names_to = "uniqueID") %>%
...
VIP_select_13v5_day3 <- cbind(clean_13v5_day3[, 1:4], 
  ...
  pivot_longer(., cols = 5:239, values_to = "log_norm_counts", names_to = "uniqueID") %>%
  group_by(., uniqueID) #cbind metadata columns (1:4) and log-transformed "clean" data for VIP. Pivot data longer and group by lipids.
...
VIP_select_13v30_day22 <- cbind(clean_13v30_day22[, 1:4], 
  ...
  pivot_longer(., cols = 5:250, values_to = "log_norm_counts", names_to = "uniqueID") %>%
  group_by(., uniqueID) #cbind metadata columns (1:4) and log-transformed "clean" data for VIP. Pivot data longer and group by lipids.
...
t.test_13v30_day22 %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>%
  nrow(.) #224 significantly different lipids
t.test_13v30_day22 %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>% filter(., estimate1 > estimate2) %>%
  nrow(.) #189 lipids higher in 13ºC
t.test_13v30_day22 %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>% filter(., estimate1 < estimate2) %>%
  nrow(.) #35 lipids higher in 30ºC
```
...
VIP_select_13v5_day22 <- cbind(clean_13v5_day22[, 1:4], 
  ...
  pivot_longer(., cols = 5:220, values_to = "log_norm_counts", names_to = "uniqueID") %>%
  group_by(., uniqueID) #cbind metadata columns (1:4) and log-transformed "clean" data for VIP. Pivot data longer and group by lipids.
...
t.test_13v5_day22 %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>%
  nrow(.) #125 significantly different lipids
t.test_13v5_day22 %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>% filter(., estimate1 > estimate2) %>%
  nrow(.) #109 lipids higher in 13ºC
t.test_13v5_day22 %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>% filter(., estimate1 < estimate2) %>%
  nrow(.) #16 lipids higher in 30ºC
```
...
VIP_select_5_day3v22 <- cbind(clean_5_day3v22[, 1:4], 
  ...
  pivot_longer(., cols = 5:219, values_to = "log_norm_counts", names_to = "uniqueID") %>%
  group_by(., uniqueID) #cbind metadata columns (1:4) and log-transformed "clean" data for VIP. Pivot data longer and group by lipids.
...
t.test_5_day3v22 %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>%
  nrow(.) #1 significantly different lipids
t.test_5_day3v22 %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>% filter(., estimate1 > estimate2) %>%
  nrow(.) #0 lipids higher in day 22
t.test_5_day3v22 %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>% filter(., estimate1 < estimate2) %>%
  nrow(.) #1 lipids higher in day 3
```
...
t.test_5_day3v22 %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>%
  left_join(., VIP_5_day3v22, by = "uniqueID") %>%
...
VIP_select_13_day3v22 <- cbind(clean_13_day3v22[, 1:4], 
  ...
  pivot_longer(., cols = 5:245, values_to = "log_norm_counts", names_to = "uniqueID") %>%
  group_by(., uniqueID) #cbind metadata columns (1:4) and log-transformed "clean" data for VIP. Pivot data longer and group by lipids.
...
VIP_select_30_day3v22 <- cbind(clean_30_day3v22[, 1:4], 
  ...
  pivot_longer(., cols = 5:256, values_to = "log_norm_counts", names_to = "uniqueID") %>%
  group_by(., uniqueID) #cbind metadata columns (1:4) and log-transformed "clean" data for VIP. Pivot data longer and group by lipids.
