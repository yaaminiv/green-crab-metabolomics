...
```{r}
loadingsPC1 <- get_loadings(lipid.ASCAmodel, component = 1, effect = 1, n_limit = 10) #Get loadings for the top 10 positively and negatively loaded compounds on the PC
head(loadingsPC1)
```
...
```{r}
loadingsPC2 <- get_loadings(lipid.ASCAmodel, component = 2, effect = 1, n_limit = 10) #Get loadings for the top 10 positively and negatively loaded compounds on the PC
head(loadingsPC2)
```
...
enrichmentResultsPC2 <- read.csv("ASCA/LION-enrichment-report-PC2/LION-enrichment-results-PC2.csv") %>%
  ...
  arrange(desc("-log(FDR)")) %>%
  slice(., 1:25) #Import enrichment results. Rename description and observed column and create new column
head(enrichmentResultsPC2) #Look at results
enrichmentResultsPC2 %>% filter(., FDR.q.value < 0.05) %>% nrow() #There are 7 significantly enriched terms
```

```{r}
enrichmentResultsPC2 %>%
  mutate(., order = rev(1:25)) %>%
  ggplot(., aes(x = `-log(FDR)`, y = as.factor(order), fill = Significant)) +
  geom_bar(stat = "identity") +
  labs(x = "-log(FDR)") +
  scale_y_discrete(name = "LION Term",
                   labels = rev(enrichmentResultsPC2$Description)) +
  scale_fill_manual(name = "FDR",
                    values = c("grey20", "springgreen4"),
                    labels = c(expression("">=0.05), expression(""<0.05))) +
  theme_classic(base_size = 15)
ggsave("ASCA/figures/PC2-logFDR.svg", heigh = 8.5, width = 11)
```
