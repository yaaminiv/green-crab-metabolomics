...
```{r}
dissim.lipid <- vegdist(scale((transLipidData[c(5:2407)] %>%
                                 log() %>%
                                 mutate_if(is.numeric, list(~na_if(., -Inf))) %>%
...
```{r}
permanova.lipid <- adonis2(dissim.lipid ~ temperature + day + temperature:day, data = transLipidData, by = "term") #Conduct global PERMANOVA to assess significance of trends in PCA. Scale lipidomics data and assess influence of treatment, day, and treatment x day on metabolite profiles
permanova.lipid #Significant influence temperature only.
```
```{r}
...
```{r}
scaled.known.pca <- prcomp(knownTransLipidData[,-c(1:4)] %>%
                       log() %>%
                       mutate_if(is.numeric, list(~na_if(., -Inf))) %>%
                       replace(is.na(.), 0), 
                     scale = FALSE, center = TRUE) #Use prcomp to perform a centered and scaled PCA calculation. Replace all -Inf with NA, and then with 0 in the dataframe and log transform prior to performing the PCA calculation
...
```{r}
dissim.lipidid <- vegdist(scale((knownTransLipidData[-c(1:4)] %>%
                                 log() %>%
                                 mutate_if(is.numeric, list(~na_if(., -Inf))) %>%
                                 replace(is.na(.), 0))), "euclidean") #Create euclidean dissimilarity matrix to use for perMANOVA and PERMDISP
```
...
```{r}
permanova.lipidid <- adonis2(dissim.lipidid ~ treatment + day + treatment:day, data = knownTransLipidData, by = "term") #Conduct global PERMANOVA to assess significance of trends in PCA. Scale lipidomics data and assess influence of treatment, day, and treatment x day on metabolite profiles
