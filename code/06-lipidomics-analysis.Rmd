...
```{r}
uniqueIDtoinChiKeyNamePubmed <- rawLipidomicsData %>%
  right_join(inChiKeyNamePubmed, ., by = "INCHIKEY") %>%
  arrange(., "PubChem.ID") %>%
  dplyr::select(., c(1:4)) #Save dataframe with all possible identifiers for downstream analysis
head(uniqueIDtoinChiKeyNamePubmed)
```
