...
cim(treatment.known.plsda, mapping = "Y", row.names = knownTransLipidData$treatment_day, xlab = "Metabolites", ylab = "Temperature (ºC)", margins = c(8, 5), row.cex = 0.5) #Create CIM plot with lipids on the x-axis and treatment x day on the y-axis. 
dev.off()
```
...
treatment.known.VIP <- as.data.frame(PLSDA.VIP(treatment.known.plsda)[["tab"]]) %>%
  ...
  left_join(., lipidIdentifiers, by = "identifier") #Extract VIP information and save as a dataframe. Convert lipid identifiers to a column. Join with lipid identifier data to get additional ID information
...
treatment.known.VIP %>%
  filter(VIP >= 1) %>%
  nrow(.) #184 lipids have VIP > 1
184/418 * 100 #44% lipids VIP > 1
```
...
treatment.known.VIP %>%
  ...
  geom_point() +
  xlab("VIP Score") + ylab("Lipid Identifier") +
  theme_classic() + theme(axis.text.y = element_text(size = 5)) #Obtain significant VIP (lipids with VIP > 1). Arrange by lipid name and reorder by cumulative VIP score. Plot points and change x and y axis labels.
