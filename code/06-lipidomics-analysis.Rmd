...
- arrange dataset by pubmed id (NAs to bottom)
- note number of lipids with name/pubmed id
- still use unique ID in the analysis
...
transLipidData <- rawLipidomicsData %>%
  ...
  left_join(lipidomicsMetadata, ., by = "crab.ID") #Remove MS information, convert lipid names to rownames, and transform dataframe. Convert rownames to crab.ID column. First 418 rows will have PubChem CID (and chemical names)
...
rowwiseLipidData <- rawLipidomicsData %>%
  ...
  slice(., c(1:612)) %>%
  log(.) %>% 
  replace(is.na(.), 0) #Take the raw lipidomics data, since that dataframe already has samples in columns and lipids in rows. Rename the metabolite column, get rid of extraneous data, and change the metabolite column to rownames. Keep only data from known lipids. Log transform the data. Replace all NA with 0.
...
All 612 lipids will be used in downstream analysis.
...
#dev.off()
```
103, 303, and 304 look like they could be outliers. I did this analysis with and without those samples and it didn't change the clustering patterns, so I'll keep them.
...
When choosing a soft thresholding power, want to choose the lowest number that reaches but does not pass the R<sup>2</sup> cutoff. From this data, it appears that the **soft thresholding power is 10**.  
...
47 241  75  72  51  28  16  12  12  11  11  11   7   7   6   5 
...
module0_lipids <- module_df %>%
  ...
  dplyr::select(., 1:5) #Take dataframe with modules and lipids, change colors to Module, and select lipids for a given module. Keep only the Lipid column. Then join with the raw lipidomics data to sequencer information.
nrow(module0_lipids) #47 lipids
head(module0_lipids)
write.csv(module0_lipids, "metaboanalyst/module0_lipids.csv", quote = FALSE, row.names = FALSE)
```
...
module1_lipids <- module_df %>%
  ...
  left_join(., (rawLipidomicsData %>% dplyr::rename(Lipid = uniqueID)), by = "Lipid") %>%
  dplyr::select(., 1:5) #Take dataframe with modules and lipids, change colors to Module, and select lipids for a given module. Keep only the Lipid column. Then join with the raw lipidomics data to sequencer information.
nrow(module1_lipids) #241 lipids
head(module1_lipids)
write.csv(module1_lipids, "metaboanalyst/module1_lipids.csv", quote = FALSE, row.names = FALSE)
```
...
module2_lipids <- module_df %>%
  ...
  left_join(., (rawLipidomicsData %>% dplyr::rename(Lipid = uniqueID)), by = "Lipid") %>%
  dplyr::select(., 1:5) #Take dataframe with modules and lipids, change colors to Module, and select lipids for a given module. Keep only the Lipid column. Then join with the raw lipidomics data to sequencer information.
nrow(module2_lipids) #75 lipids
head(module2_lipids)
write.csv(module2_lipids, "metaboanalyst/module2_lipids.csv", quote = FALSE, row.names = FALSE)
```
...
module3_lipids <- module_df %>%
  ...
  left_join(., (rawLipidomicsData %>% dplyr::rename(Lipid = uniqueID)), by = "Lipid") %>%
  dplyr::select(., 1:5) #Take dataframe with modules and lipids, change colors to Module, and select lipids for a given module. Keep only the Lipid column. Then join with the raw lipidomics data to sequencer information.
nrow(module3_lipids) #72 lipids
head(module3_lipids)
write.csv(module3_lipids, "metaboanalyst/module3_lipids.csv", quote = FALSE, row.names = FALSE)
```
...
module4_lipids <- module_df %>%
  ...
  left_join(., (rawLipidomicsData %>% dplyr::rename(Lipid = uniqueID)), by = "Lipid") %>%
  dplyr::select(., 1:5) #Take dataframe with modules and lipids, change colors to Module, and select lipids for a given module. Keep only the Lipid column. Then join with the raw lipidomics data to sequencer information.
nrow(module4_lipids) #51 lipids
head(module4_lipids)
write.csv(module4_lipids, "metaboanalyst/module4_lipids.csv", quote = FALSE, row.names = FALSE)
```
...
module5_lipids <- module_df %>%
  ...
  left_join(., (rawLipidomicsData %>% dplyr::rename(Lipid = uniqueID)), by = "Lipid") %>%
  dplyr::select(., 1:5) #Take dataframe with modules and lipids, change colors to Module, and select lipids for a given module. Keep only the Lipid column. Then join with the raw lipidomics data to sequencer information.
nrow(module5_lipids) #28 lipids
head(module5_lipids)
write.csv(module5_lipids, "metaboanalyst/module5_lipids.csv", quote = FALSE, row.names = FALSE)
```
...
module6_lipids <- module_df %>%
  ...
  left_join(., (rawLipidomicsData %>% dplyr::rename(Lipid = uniqueID)), by = "Lipid") %>%
  dplyr::select(., 1:5) #Take dataframe with modules and lipids, change colors to Module, and select lipids for a given module. Keep only the Lipid column. Then join with the raw lipidomics data to sequencer information.
nrow(module6_lipids) #16 lipids
head(module6_lipids)
write.csv(module6_lipids, "metaboanalyst/module6_lipids.csv", quote = FALSE, row.names = FALSE)
```
...
module7_lipids <- module_df %>%
  ...
  left_join(., (rawLipidomicsData %>% dplyr::rename(Lipid = uniqueID)), by = "Lipid") %>%
  dplyr::select(., 1:5) #Take dataframe with modules and lipids, change colors to Module, and select lipids for a given module. Keep only the Lipid column. Then join with the raw lipidomics data to sequencer information.
nrow(module7_lipids) #12 lipids
head(module7_lipids)
write.csv(module7_lipids, "metaboanalyst/module7_lipids.csv", quote = FALSE, row.names = FALSE)
```
...
module8_lipids <- module_df %>%
  ...
  left_join(., (rawLipidomicsData %>% dplyr::rename(Lipid = uniqueID)), by = "Lipid") %>%
  dplyr::select(., 1:5) #Take dataframe with modules and lipids, change colors to Module, and select lipids for a given module. Keep only the Lipid column. Then join with the raw lipidomics data to sequencer information.
nrow(module8_lipids) #12 lipids
head(module8_lipids)
write.csv(module8_lipids, "metaboanalyst/module8_lipids.csv", quote = FALSE, row.names = FALSE)
```
...
module9_lipids <- module_df %>%
  ...
  left_join(., (rawLipidomicsData %>% dplyr::rename(Lipid = uniqueID)), by = "Lipid") %>%
  dplyr::select(., 1:5) #Take dataframe with modules and lipids, change colors to Module, and select lipids for a given module. Keep only the Lipid column. Then join with the raw lipidomics data to sequencer information.
nrow(module9_lipids) #11 lipids
head(module9_lipids)
write.csv(module9_lipids, "metaboanalyst/module9_lipids.csv", quote = FALSE, row.names = FALSE)
```
...
module10_lipids <- module_df %>%
  ...
  left_join(., (rawLipidomicsData %>% dplyr::rename(Lipid = uniqueID)), by = "Lipid") %>%
  dplyr::select(., 1:5) #Take dataframe with modules and lipids, change colors to Module, and select lipids for a given module. Keep only the Lipid column. Then join with the raw lipidomics data to sequencer information.
nrow(module10_lipids) #11 lipids
head(module10_lipids)
write.csv(module10_lipids, "metaboanalyst/module10_lipids.csv", quote = FALSE, row.names = FALSE)
```
...
module11_lipids <- module_df %>%
  ...
  left_join(., (rawLipidomicsData %>% dplyr::rename(Lipid = uniqueID)), by = "Lipid") %>%
  dplyr::select(., 1:5) #Take dataframe with modules and lipids, change colors to Module, and select lipids for a given module. Keep only the Lipid column. Then join with the raw lipidomics data to sequencer information.
nrow(module11_lipids) #11 lipids
head(module11_lipids)
write.csv(module11_lipids, "metaboanalyst/module11_lipids.csv", quote = FALSE, row.names = FALSE)
```
...
module12_lipids <- module_df %>%
  ...
  left_join(., (rawLipidomicsData %>% dplyr::rename(Lipid = uniqueID)), by = "Lipid") %>%
  dplyr::select(., 1:5) #Take dataframe with modules and lipids, change colors to Module, and select lipids for a given module. Keep only the Lipid column. Then join with the raw lipidomics data to sequencer information.
nrow(module12_lipids) #7 lipids
head(module12_lipids)
write.csv(module12_lipids, "metaboanalyst/module12_lipids.csv", quote = FALSE, row.names = FALSE)
```
...
module13_lipids <- module_df %>%
  ...
  left_join(., (rawLipidomicsData %>% dplyr::rename(Lipid = uniqueID)), by = "Lipid") %>%
  dplyr::select(., 1:5) #Take dataframe with modules and lipids, change colors to Module, and select lipids for a given module. Keep only the Lipid column. Then join with the raw lipidomics data to sequencer information.
nrow(module13_lipids) #7 lipids
head(module13_lipids)
write.csv(module13_lipids, "metaboanalyst/module13_lipids.csv", quote = FALSE, row.names = FALSE)
```
...
module14_lipids <- module_df %>%
  ...
  left_join(., (rawLipidomicsData %>% dplyr::rename(Lipid = uniqueID)), by = "Lipid") %>%
  dplyr::select(., 1:5) #Take dataframe with modules and lipids, change colors to Module, and select lipids for a given module. Keep only the Lipid column. Then join with the raw lipidomics data to sequencer information.
nrow(module14_lipids) #6 lipids
head(module14_lipids)
write.csv(module14_lipids, "metaboanalyst/module14_lipids.csv", quote = FALSE, row.names = FALSE)
```
...
module15_lipids <- module_df %>%
  ...
  left_join(., (rawLipidomicsData %>% dplyr::rename(Lipid = uniqueID)), by = "Lipid") %>%
  dplyr::select(., 1:5) #Take dataframe with modules and lipids, change colors to Module, and select lipids for a given module. Keep only the Lipid column. Then join with the raw lipidomics data to sequencer information.
nrow(module15_lipids) #5 lipids
head(module15_lipids)
write.csv(module15_lipids, "metaboanalyst/module15_lipids.csv", quote = FALSE, row.names = FALSE)
```
...
The output tables (msea_ora_result.csv files from MetaboAnalyst) were renamed according to module number and saved [here](https://github.com/yaaminiv/green-crab-lipidomics/tree/main/output/05-lipidomics-analysis/metaboanalyst/msea-compoundClass/enrichment-output).
