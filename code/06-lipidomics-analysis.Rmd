...
Before analyzing the data, I will transform the data and match with treatment metadata. The West Coast Metabolomics Center has already normalized data. 
...
transLipidData <- rawLipidomicsData %>%
  ...
  left_join(lipidomicsMetadata, ., by = "crab.ID") #Take raw data and group by identifier. Average abundance across numeric columns to deal with any duplicate entries. Transpose data, convert the first row (identifiers) to column names, and remove this row. Convert rownames to crab ID colunn. Join with lipidomics metadata. 
transLipidData[, c(4:2410)] <- transLipidData[, c(4:2410)] %>% mutate_if(is.character, as.numeric) #Convert data columns to numeric
...
```{r}
scaled.pca <- prcomp(log(transLipidData[c(5:2410)]) %>%
                       replace(is.na(.), 0), 
                     scale = TRUE, center = TRUE) #Use prcomp to perform a centered and scaled PCA calculation. Replace all NA with 0 in the dataframe and log transform prior to performing the PCA calculation
...
ordiMonteRandTest <- ordi.monte((log(transLipidData[c(5:2410)]) %>%
                                   replace(is.na(.), 0)), ord = "pca", dim = 6, plot = FALSE) #Conduct monte carlo randomization test using 1000 random permutations of the input data.
ordiMonteRandTest
...
```{r}
permanova.lipid <- adonis2(scale((log(transLipidData[c(5:2410)]) %>%
...
The next step is for me to conduct a PLS-DA analysis using the `mixOmics` package. This is a more robust way to analyze differences between treatments. Since both temperature and day were significant in the PCA, I'll use both of those factors in the PLSDA.
...
```{r}
X <- log(transLipidData[c(5:2410)]) #Assign only metabolite data to the X dataframe after log transformation.
...
plotIndiv(treatment.plsda, comp = c(1,2), ind.names = FALSE,
          ...
          title = "PLS-DA by Temperature", X.label = "Component 1: 31% Explained Variance", size.xlabel = 15, Y.label = "Component 2: 11% Explained Variance", size.ylabel = 15, size.axis = 13,
...
treatmentVIP <- as.data.frame(PLSDA.VIP(treatment.plsda)[["tab"]]) %>%
  rownames_to_column(., var = "uniqueID") #Extract VIP information and save as a dataframe. Convert metabolite rownames to a column. 
...
treatmentVIP %>%
  filter(VIP >= 1) %>%
  nrow(.) #1005 lipids have VIP > 1
1005/2406 * 100 #41.8% lipids VIP > 1
```
...
treatmentVIP %>%
  filter(VIP >= 1) %>%
  arrange(VIP) %>%
  ggplot(aes(x = VIP, y = reorder(uniqueID, VIP, sum))) +
  geom_point() +
  xlab("VIP Score") + ylab("Unique Lipid ID") +
...
```{r}
X <- log(transLipidData[c(5:2410)]) #Assign only metabolite data to the X dataframe after log transformation
Y <- as.factor(transLipidData$treatment_day) #Assign treatment data to the Y dataframe
```
...
plotIndiv(treatmentday.plsda, comp = c(1,2), ind.names = FALSE,
          cex = 3, col = c("grey80", "#525252", "salmon", "#CB181D", "lightblue", "#2171B5"), pch = c(12:17), point.lwd = 0.7,
          ellipse = TRUE, star = TRUE, centroid = FALSE, 
          legend = TRUE, legend.title = "Temperature (ºC) x Day", 
          title = "PLS-DA by Temperature and Day", X.label = "Component 1: 31% Explained Variance", size.xlabel = 15, Y.label = "Component 2: 11% Explained Variance", size.ylabel = 15, size.axis = 13,
          style = "ggplot2") #Visualize first two components of PLS-DA with no individual sample names. Alter point size, color, shape, and line width. Include 95% confidence ellipses and use star format to visualize sample distance from centroids. Include a legend and specify the legend title. Specify the plot title and text label sizes. Use ggplot style.
#dev.off()
```
...
cim(treatment.plsda, mapping = "Y", row.names = transLipidData$treatment_day, xlab = "Unique Lipid ID", ylab = "Temperature (ºC) x Day", margins = c(8, 5), row.cex = 0.5) #Create CIM plot with lipids on the x-axis and treatment x day on the y-axis. 
dev.off()
```
...
treatmentdayVIP %>%
  filter(VIP >= 1) %>%
  nrow(.) #251 lipids have VIP > 1
972/2406 * 100 #40.4% lipids VIP > 1
```
...
```{r}
X <- log(transLipidData[c(5:423)]) #Assign known metabolite data to the X dataframe after log transformation
Y <- as.factor(transLipidData$treatment_day) #Assign treatment data to the Y dataframe
```
...
```{r}
#pdf("PLSDA/figures/known-lipids-PLSDA-day-treatment-comp12.pdf", height = 8.5, width = 11)
plotIndiv(treatmentday.known.plsda, comp = c(1,2), ind.names = FALSE,
          cex = 3, col = c("grey80", "#525252", "salmon", "#CB181D", "lightblue", "#2171B5"), pch = c(12:17), point.lwd = 0.7,
          ellipse = TRUE, star = TRUE, centroid = FALSE, 
          legend = TRUE, legend.title = "Temperature (ºC) x Day", 
          title = "PLS-DA by Temperature and Day", X.label = "Component 1: 26% Explained Variance", size.xlabel = 15, Y.label = "Component 2: 17% Explained Variance", size.ylabel = 15, size.axis = 13,
          style = "ggplot2") #Visualize first two components of PLS-DA with no individual sample names. Alter point size, color, shape, and line width. Include 95% confidence ellipses and use star format to visualize sample distance from centroids. Include a legend and specify the legend title. Specify the plot title and text label sizes. Use ggplot style.
#dev.off()
```

```{r}
#pdf("PLSDA/figures/known-lipids-PLSDA-day-treatment-comp13.pdf", height = 8.5, width = 11)
plotIndiv(treatmentday.known.plsda, comp = c(1,3), ind.names = FALSE,
          ...
          ellipse = TRUE, star = TRUE, centroid = FALSE, 
          legend = TRUE, legend.title = "Temperature (ºC) x Day", 
          title = "PLS-DA by Temperature and Day", X.label = "Component 1: 26% Explained Variance", size.xlabel = 15, Y.label = "Component 3: 8% Explained Variance", size.ylabel = 15, size.axis = 13,
          style = "ggplot2") #Visualize first two components of PLS-DA with no individual sample names. Alter point size, color, shape, and line width. Include 95% confidence ellipses and use star format to visualize sample distance from centroids. Include a legend and specify the legend title. Specify the plot title and text label sizes. Use ggplot style.
#dev.off()
```

```{r}
pdf("PLSDA/figures/known-lipids-cimPlot-day-treatment.pdf", height = 8.5, width = 11)
cim(treatmentday.known.plsda, mapping = "Y", row.names = transLipidData$treatment_day, xlab = "Metabolites", ylab = "Temperature (ºC) x Day", margins = c(8, 5), row.cex = 0.5) #Create CIM plot with lipids on the x-axis and treatment x day on the y-axis. 
dev.off()
```
...
```{r}
write.csv(treatmentday.known.VIP, "PLSDA/known-lipids-VIP-day-treatment.csv", quote = FALSE, row.names = FALSE) #Save VIP scores
```

```{r}
treatmentday.known.VIP %>%
  filter(VIP >= 1) %>%
  nrow(.) #187 lipids have VIP > 1
187/418 * 100 #44.7% lipids VIP > 1
```
...
treatmentday.known.VIP %>%
  ...
  geom_point() +
  xlab("VIP Score") + ylab("Unique Lipid ID") +
  theme_classic() + theme(axis.text.y = element_text(size = 8)) #Using only lipids with VIP > 1, arrange by metabolite name and reorder by cumulative VIP score. Plot points and change x and y axis labels.
ggsave("PLSDA/figures/known-lipids-VIP-day-treatment.pdf", height = 8.5, width = 11)
```

## Pairwise tests for VIP lipids

We now know which lipids are most important for separating days and treatments. However, we do not know specifically which VIP distinguish between which treatment x day contrasts. I will use pairwise t-tests to identify which VIP are important for which contrasts. 
...
X <- transLipidData %>%
  ...
  droplevels() #Filter transLipidData for desired contrasts
Y <- as.factor(X$treatment_day) #Treatments for Y
X <- X[, 5:423] #Retain data only
```
...
```{r}
clean_13v30_day3 <- transLipidData %>%
  dplyr::filter(., treatment_day %in% list) %>%
  droplevels() #Filter transLipidData for desired contrasts.
VIP_select_13v30_day3 <- cbind(clean_13v30_day3[, 1:4], 
                               log(clean_13v30_day3[, names(clean_13v30_day3) %in% VIP_13v30_day3$uniqueID])) %>%
  pivot_longer(., cols = 5:185, values_to = "log_norm_counts", names_to = "uniqueID") %>%
  group_by(., uniqueID) #cbind metadata columns (1:4) and log-transformed "clean" data for VIP. Pivot data longer and group by lipids.
...
t.test_13v30_day3$p.adj <- p.adjust(t.test_13v30_day3$p.value, method = c("fdr"), n = length(VIP_13v30_day3$uniqueID)) #Adjust p value for the number of comparisons
t.test_13v30_day3
```
...
t.test_13v30_day3 %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>%
  nrow(.) #0 significantly different lipids
```
...
```{r}
clean_13v5_day3 <- transLipidData %>%
  dplyr::filter(., treatment_day %in% list) %>%
  droplevels() #Filter transLipidData for desired contrasts.
VIP_select_13v5_day3 <- cbind(clean_13v5_day3[, 1:4], 
                              log(clean_13v5_day3[, names(clean_13v5_day3) %in% VIP_13v5_day3$uniqueID])) %>%
  pivot_longer(., cols = 5:162, values_to = "log_norm_counts", names_to = "uniqueID") %>%
  group_by(., uniqueID) #cbind metadata columns (1:4) and log-transformed "clean" data for VIP. Pivot data longer and group by lipids.
...
t.test_13v5_day3$p.adj <- p.adjust(t.test_13v5_day3$p.value, method = c("fdr"), n = length(VIP_13v5_day3$uniqueID)) #Adjust p value for the number of comparisons
t.test_13v5_day3
```
...
VIP_13v30_day22 <- as.data.frame(PLSDA.VIP(known.plsda_13v30_day22)[["tab"]]) %>%
  rownames_to_column(., var = "uniqueID") %>%
  filter(., VIP >= 1) #Extract VIP list and filter for VIP > 1
nrow(VIP_13v30_day22)
```

```{r}
clean_13v30_day22 <- transLipidData %>%
  dplyr::filter(., treatment_day %in% list) %>%
  droplevels() #Filter transLipidData for desired contrasts.
VIP_select_13v30_day22 <- cbind(clean_13v30_day22[, 1:4], 
                                log(clean_13v30_day22[, names(clean_13v30_day22) %in% VIP_13v30_day22$uniqueID])) %>%
  pivot_longer(., cols = 5:170, values_to = "log_norm_counts", names_to = "uniqueID") %>%
  group_by(., uniqueID) #cbind metadata columns (1:4) and log-transformed "clean" data for VIP. Pivot data longer and group by lipids.
...
t.test_13v30_day22$p.adj <- p.adjust(t.test_13v30_day22$p.value, method = c("fdr"), n = length(VIP_13v30_day22$uniqueID)) #Adjust p value for the number of comparisons
t.test_13v30_day22
```
...
t.test_13v30_day22 %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>%
  nrow(.) #154 significantly different lipids
t.test_13v30_day22 %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>% filter(., estimate1 > estimate2) %>%
  nrow(.) #121 lipids higher in 13ºC
t.test_13v30_day22 %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>% filter(., estimate1 < estimate2) %>%
...
t.test_13v30_day22 %>%
  filter(., p.adj < 0.05) %>%
  right_join(x = uniqueIDtoinChiKeyNamePubmed, y = ., by = "uniqueID") %>%
  write.csv("PLSDA/known-lipids-13v30-day22-VIP.csv", quote = FALSE, row.names = FALSE) #Take t-test results and join with unique ID information to get chemical names. Save as a csv
```
...
t.test_13v30_day22 %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>%
  left_join(., VIP_13v30_day22, by = "uniqueID") %>%
  mutate(., direction = case_when(estimate > 1 ~ "13",
                                  estimate < 1 ~ "30")) %>%
  mutate(., score = case_when(estimate > 1 ~ VIP,
                              estimate < 1 ~ -VIP)) %>%
  arrange(score) %>%
  ggplot(aes(x = score, y = reorder(uniqueID, score, sum))) +
  ...
  scale_fill_manual(values = c(plotColors[2], plotColors[1]), 
                    ...
                    breaks = c("13", "30"),
                    labels = c("13", "30")) +
  theme_classic(base_size = 15) + theme() #Take t-test results and filter out significant lipids. Create two new columns using estimate values: one for the direction of increase, and another for positive or negative VIP. Arrange by uniqueID name and reorder by cumulative VIP score. Plot bars and change x and y axis labels. Add scale information.
...
heatmapAnnot <- lipidomicsMetadata %>%
  ...
  dplyr::select(., Temperature, Day) #Retain treatment and day information only
head(heatmapAnnot) #Column annotation information
```

```{r}
heatmapAnnotColors <- list(Day = c("3" = "grey80",
                                   "22" = "grey20"),
                           Temperature = c("5" = plotColors[1],
                                           "13" = plotColors[2],
                                           "30" = plotColors[3])) #List of colors for column annotations
heatmapAnnotColors #Column annotation colors
```
...
```{r}
clean_13v5_day22 <- transLipidData %>%
  dplyr::filter(., treatment_day %in% list) %>%
  droplevels() #Filter transLipidData for desired contrasts.
VIP_select_13v5_day22 <- cbind(clean_13v5_day22[, 1:4], 
                               log(clean_13v5_day22[, names(clean_13v5_day22) %in% VIP_13v5_day22$uniqueID])) %>%
  pivot_longer(., cols = 5:164, values_to = "log_norm_counts", names_to = "uniqueID") %>%
  group_by(., uniqueID) #cbind metadata columns (1:4) and log-transformed "clean" data for VIP. Pivot data longer and group by lipids.
VIP_select_13v5_day22
```
...
t.test_13v5_day22$p.adj <- p.adjust(t.test_13v5_day22$p.value, method = c("fdr"), n = length(VIP_13v5_day22$uniqueID)) #Adjust p value for the number of comparisons
t.test_13v5_day22
```
...
t.test_13v5_day22 %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>%
  nrow(.) #89 significantly different lipids
t.test_13v5_day22 %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>% filter(., estimate1 > estimate2) %>%
  nrow(.) #77 lipids higher in 13ºC
t.test_13v5_day22 %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>% filter(., estimate1 < estimate2) %>%
  nrow(.) #12 lipids higher in 5ºC
```
...
t.test_13v5_day22 %>%
  filter(., p.adj < 0.05) %>%
  right_join(x = uniqueIDtoinChiKeyNamePubmed, y = ., by = "uniqueID") %>%
  write.csv("PLSDA/known-lipids-13v5-day22-VIP.csv", quote = FALSE, row.names = FALSE) #Take t-test results and join with unique ID information to get chemical names. Save as a csv
```
...
t.test_13v5_day22 %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>%
  left_join(., VIP_13v5_day22, by = "uniqueID") %>%
  mutate(., direction = case_when(estimate > 1 ~ "13",
                                  estimate < 1 ~ "5")) %>%
  mutate(., score = case_when(estimate > 1 ~ VIP,
                              estimate < 1 ~ -VIP)) %>%
...
```{r}
clean_5_day3v22 <- transLipidData %>%
  dplyr::filter(., treatment_day %in% list) %>%
  droplevels() #Filter transLipidData for desired contrasts.
VIP_select_5_day3v22 <- cbind(clean_5_day3v22[, 1:4], 
                              log(clean_5_day3v22[, names(clean_5_day3v22) %in% VIP_5_day3v22$uniqueID])) %>%
  pivot_longer(., cols = 5:156, values_to = "log_norm_counts", names_to = "uniqueID") %>%
  group_by(., uniqueID) #cbind metadata columns (1:4) and log-transformed "clean" data for VIP. Pivot data longer and group by lipids.
...
t.test_5_day3v22$p.adj <- p.adjust(t.test_5_day3v22$p.value, method = c("fdr"), n = length(VIP_5_day3v22$uniqueID)) #Adjust p value for the number of comparisons
t.test_5_day3v22
```
...
```{r}
clean_13_day3v22 <- transLipidData %>%
  dplyr::filter(., treatment_day %in% list) %>%
  droplevels() #Filter transLipidData for desired contrasts.
VIP_select_13_day3v22 <- cbind(clean_13_day3v22[, 1:4], 
                               log(clean_13_day3v22[, names(clean_13_day3v22) %in% VIP_13_day3v22$uniqueID])) %>%
  pivot_longer(., cols = 5:180, values_to = "log_norm_counts", names_to = "uniqueID") %>%
  group_by(., uniqueID) #cbind metadata columns (1:4) and log-transformed "clean" data for VIP. Pivot data longer and group by lipids.
...
t.test_13_day3v22$p.adj <- p.adjust(t.test_13_day3v22$p.value, method = c("fdr"), n = length(VIP_13_day3v22$uniqueID)) #Adjust p value for the number of comparisons
t.test_13_day3v22
```
...
```{r}
clean_30_day3v22 <- transLipidData %>%
  dplyr::filter(., treatment_day %in% list) %>%
  droplevels() #Filter transLipidData for desired contrasts.
VIP_select_30_day3v22 <- cbind(clean_30_day3v22[, 1:4], 
                               log(clean_30_day3v22[, names(clean_30_day3v22) %in% VIP_30_day3v22$uniqueID])) %>%
  pivot_longer(., cols = 5:174, values_to = "log_norm_counts", names_to = "uniqueID") %>%
  group_by(., uniqueID) #cbind metadata columns (1:4) and log-transformed "clean" data for VIP. Pivot data longer and group by lipids.
...
t.test_30_day3v22$p.adj <- p.adjust(t.test_30_day3v22$p.value, method = c("fdr"), n = length(VIP_30_day3v22$uniqueID)) #Adjust p value for the number of comparisons
t.test_30_day3v22
```
...
t.test_30_day3v22 %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>%
  nrow(.) #1 significantly different lipid
```
...
t.test_30_day3v22 %>%
  filter(., p.adj < 0.05) %>%
  right_join(x = uniqueIDtoinChiKeyNamePubmed, y = ., by = "uniqueID") %>%
  write.csv("PLSDA/known-lipids-30-day3v22-VIP.csv", quote = FALSE, row.names = FALSE) #Take t-test results and join with unique ID information to get chemical names. Save as a csv
```
...
t.test_30_day3v22 %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>%
  left_join(., VIP_30_day3v22, by = "uniqueID") %>%
  ...
  scale_fill_manual(values = c("black", "purple"), 
                    ...
                    labels = c("3", "22")) +
  theme_classic(base_size = 15) + theme() #Take t-test results and filter out significant lipids. Create two new columns using estimate values: one for the direction of increase, and another for positive or negative VIP. Arrange by uniqueID name and reorder by cumulative VIP score. Plot bars and change x and y axis labels. Add scale information.
ggsave("PLSDA/figures/significant-VIP-30-day3v22.pdf", height = 8.5, width = 11)
```
...
plotIndiv(myResult.splsda.final_T, comp = c(1,2), ind.names = FALSE,
          ...
          ellipse = TRUE, star = TRUE, centroid = FALSE, 
          legend = TRUE, legend.title = "Temperature (ºC) x Day", 
          title = "sPLS-DA by Temperature and Day", X.label = "Component 1: 14% Explained Variance", size.xlabel = 15, Y.label = "Component 2: 22% Explained Variance", size.ylabel = 15, size.axis = 13,
          style = "ggplot2")
#dev.off()
```
...
```{r}
rowwiseLipidData <- rawLipidomicsData %>%
  right_join(inChiKeyNamePubmed, ., by = "INCHIKEY") %>%
  arrange(., "PubChem.ID") %>%
  slice(., c(1:418)) %>%
  ...
  log(.) %>% 
  replace(is.na(.), 0) #Take the raw lipidomics data, since that dataframe already has samples in columns and lipids in rows. Join with inChiKey table and arrange by CID. Keep the first 418 rows, since those are the known lipids (names and CID included). Change the unique IDs to rownames.  Log transform the data. Replace all NA with 0.
tail(rowwiseLipidData) #Confirm formatting
```
...
*pOverA*: Specifying the minimum count for a proportion of samples for each metabolite. Here, we are using a pOverA of 0.08. This is because we have 69 samples with a minimum of n = 6 samples per treatment x day. Therefore, we will accept lipids that are present in 6/69 = 0.08 of the samples because we expect different lipids by temperature and day as demonstrated by PLSDA analysis. We are further setting the minimum value of lipids to 0.01 (median normalized), such that 8% of the samples must have a non-zero normalized metabolite presence in order for the metabolite to remain in the data set.  
...
All 418 known lipids will be used in downstream analysis.
...
There's so much data that it's hard to tell if anything is actually an outlier, but most if not all lipids seem to cluster with other lipids.
...
303, 304, and 606 look like they could be outliers (all from 30ºC treatment at day 3). I did this analysis with and without those samples and it significantly changed the sample clustering, so I'll proceed with outlier removal.

```{r}
rowwiseLipidData[-c(30, 31, 67),]
```
...
sft <-pickSoftThreshold(rowwiseLipidData[-c(30, 31, 67),], powerVector = powers, verbose = 5) # Call the network topology analysis function
```
...
netwk <- blockwiseModules(rowwiseLipidData[-c(30, 31, 67),],                         # <= input here
...
12 144  54  40  26  23  22  22  16  11  10   9   8   8   7   6 
...
nMetabolites <- ncol(rowwiseLipidData[-c(30, 31, 67),])
nSamples <- nrow(rowwiseLipidData[-c(30, 31, 67),])

nMetabolites
nSamples
```

We have 612 lipids and 69 samples.   
...
lipidomicsMetadata$treatment_day <- as.factor(lipidomicsMetadata$treatment_day)
```
...
## Comparing with Chapelle results

I want to do a rough analysis with phosphatidylethanolamine (PE) and phosphatidylcholines (PC) compounds, since Chapelle 1978 and 1986 found them most responsive to temperature.
...
PEData <- rawLipidomicsData %>%
  ...
  t(.) %>% as.data.frame(.) %>%
  rownames_to_column(., var = "crab.ID") %>%
  left_join(lipidomicsMetadata, ., by = "crab.ID") #Slice PE data from the raw dataframe and remove extraneous columns. Unite lipid and uniqueID column to make unique column names. Transpose dataframe and convert crab ID to a column. Join with metadata
head(PEData) #Confirm dataframe creation
```

```{r}
# pdf("metaboanalyst/figures/PE-heatmap.pdf", width = 11, height = 8.5)
pheatmap(t(log(PEData[, 5:61])), cluster_row = TRUE, show_rownames = FALSE, cluster_cols = TRUE, show_colnames = FALSE, annotation_col = heatmapAnnot)
# dev.off()
```
...
pheatmap(t(log(TGData[, 5:61])), cluster_row = TRUE, show_rownames = FALSE, cluster_cols = FALSE, show_colnames = FALSE, annotation_col = heatmapAnnot)
# dev.off()
```
...
For each module, I want the lipid unique ID name, but I also want all of the information provided by the sequencing facility. This is because common names for lipids can be different, and using a numerical classifier will ensure the correct compounds are recognized by MetaboAnalyst.
...
module0_lipids <- module_df %>%
  ...
  filter(., Module == 0) %>%
  dplyr::select(., Lipid) %>% 
  left_join(., (rawLipidomicsData %>% dplyr::rename(Lipid = uniqueID)), by = "Lipid") %>%
  right_join(inChiKeyNamePubmed, ., by = "INCHIKEY") %>%
  dplyr::select(., 1:6) #Take dataframe with modules and lipids, change colors to Module, and select lipids for a given module. Keep only the Lipid column. Then join with the raw lipidomics data to sequencer information, and inChiKey table for additional annotations
nrow(module0_lipids) #31 lipids
head(module0_lipids)
...
module1_lipids <- module_df %>%
  ...
  right_join(inChiKeyNamePubmed, ., by = "INCHIKEY") %>%
  dplyr::select(., 1:6) #Take dataframe with modules and lipids, change colors to Module, and select lipids for a given module. Keep only the Lipid column. Then join with the raw lipidomics data to sequencer information, and inChiKey table for additional annotations
nrow(module1_lipids) #157 lipids
head(module1_lipids)
...
module10_lipids <- module_df %>%
  ...
  right_join(inChiKeyNamePubmed, ., by = "INCHIKEY") %>%
  dplyr::select(., 1:6) #Take dataframe with modules and lipids, change colors to Module, and select lipids for a given module. Keep only the Lipid column. Then join with the raw lipidomics data to sequencer information, and inChiKey table for additional annotations
nrow(module10_lipids) #10 lipids
