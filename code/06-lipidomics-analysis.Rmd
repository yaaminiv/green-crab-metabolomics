...
module0_lipids <- module_df %>%
  ...
  left_join(., (rawLipidomicsData %>% dplyr::rename(Metabolite = BinBase.name)), by = "Metabolite") %>%
  dplyr::select(., Metabolite, PubChem, KEGG) #Take dataframe with modules and lipids, change colors to Module, and select lipids for a given module. Keep only the metabolite column. Then join with the raw lipidomics data to get PubChem and KEGG ids.
nrow(module0_lipids) #40 lipids
head(module0_lipids)
