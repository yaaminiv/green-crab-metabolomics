...
treatmentVIP <- as.data.frame(PLSDA.VIP(treatment.plsda)[["tab"]]) %>%
  rownames_to_column(., var = "identifier") #Extract VIP information and save as a dataframe. Convert metabolite rownames to a column. 
...
treatmentVIP %>%
  filter(VIP >= 1) %>%
  arrange(VIP) %>%
  ggplot(aes(x = VIP, y = reorder(identifier, VIP, sum))) +
  geom_point() +
  xlab("VIP Score") + ylab("Lipid Identifier") +
