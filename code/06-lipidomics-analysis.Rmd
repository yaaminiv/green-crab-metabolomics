...
knownTransLipidData <- transLipidData %>%
  dplyr::select(1:4, which(colnames(transLipidData) %in% lipidIdentifiers$identifier[1:418])) #Take transposed lipid data and select columns associated with known lipids.
...
plotIndiv(treatment.known.plsda)
```
...
plotIndiv(treatment.known.plsda, comp = c(1,2), ind.names = FALSE,
          cex = 3, col = c("#2171B5", "#525252", "#CB181D"), pch = c(15, 17, 19), point.lwd = 0.7,
          ellipse = TRUE, star = TRUE, centroid = FALSE, 
          legend = TRUE, legend.title = "Temperature (ºC)", 
          title = "PLS-DA by Temperature", X.label = "Component 1: 26% Explained Variance", size.xlabel = 15, Y.label = "Component 2: 18% Explained Variance", size.ylabel = 15, size.axis = 13,
          style = "ggplot2") #Visualize first two components of PLS-DA with no individual sample names. Alter point size, color, shape, and line width. Include 95% confidence ellipses and use star format to visualize sample distance from centroids. Include a legend and specify the legend title. Specify the plot title and text label sizes. Use ggplot style.
#dev.off()
```
```{r}
...
plotIndiv(treatment.known.plsda, comp = c(1,3), ind.names = FALSE,
          ...
          ellipse = TRUE, star = TRUE, centroid = FALSE, 
          legend = TRUE, legend.title = "Temperature (ºC)", 
          title = "PLS-DA by Temperature", X.label = "Component 1: 26% Explained Variance", size.xlabel = 15, Y.label = "Component 3: 8% Explained Variance", size.ylabel = 15, size.axis = 13,
          style = "ggplot2") #Visualize first two components of PLS-DA with no individual sample names. Alter point size, color, shape, and line width. Include 95% confidence ellipses and use star format to visualize sample distance from centroids. Include a legend and specify the legend title. Specify the plot title and text label sizes. Use ggplot style.
#dev.off()
```
...
cim(treatment.known.plsda, mapping = "Y", row.names = knownTransLipidData$treatment_day, xlab = "Metabolites", ylab = "Temperature (ºC) x Day", margins = c(8, 5), row.cex = 0.5) #Create CIM plot with lipids on the x-axis and treatment x day on the y-axis. 
dev.off()
```
...
```{r}
write.csv(treatment.known.VIP, "PLSDA/known-lipids-VIP-day-treatment.csv", quote = FALSE, row.names = FALSE) #Save VIP scores
```
...
```{r}
clean_13v30_day3 <- knownTransLipidData %>%
  dplyr::filter(., treatment_day %in% list) %>%
  droplevels() #Filter knownTransLipidData for desired contrasts.
...
```{r}
clean_13v5_day3 <- knownTransLipidData %>%
  dplyr::filter(., treatment_day %in% list) %>%
  droplevels() #Filter knownTransLipidData for desired contrasts.
...
```{r}
clean_13v30_day22 <- knownTransLipidData %>%
  dplyr::filter(., treatment_day %in% list) %>%
  droplevels() #Filter knownTransLipidData for desired contrasts.
...
```{r}
clean_13v5_day22 <- knownTransLipidData %>%
  dplyr::filter(., treatment_day %in% list) %>%
  droplevels() #Filter knownTransLipidData for desired contrasts.
...
```{r}
clean_5_day3v22 <- knownTransLipidData %>%
  dplyr::filter(., treatment_day %in% list) %>%
  droplevels() #Filter knownTransLipidData for desired contrasts.
...
```{r}
clean_13_day3v22 <- knownTransLipidData %>%
  dplyr::filter(., treatment_day %in% list) %>%
  droplevels() #Filter knownTransLipidData for desired contrasts.
...
```{r}
clean_30_day3v22 <- knownTransLipidData %>%
  dplyr::filter(., treatment_day %in% list) %>%
  droplevels() #Filter knownTransLipidData for desired contrasts.
...
X <- log(knownTransLipidData[c(5:423)]) #Assign known metabolite data to the X dataframe after log transformation
Y <- as.factor(knownTransLipidData$treatment_day) #Assign treatment data to the Y dataframe
```
