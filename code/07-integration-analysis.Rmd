...
all_MO2_slope_results_sampled <- all_MO2_slope_results_demTTRdata %>%
  ...
  dplyr::select(-c(TTRavg:TTRSE)) %>%
  mutate(., crab.ID = as.factor(crab.ID)) #Change treatment, day, and treatment_day columns to match metabolomics/lipidomics datasets. Retain only respirometry data on the days that sampling was done. Join with modTTRSampled by all columns in common to get crab.ID. Remove superfluous columns. 
...
datTraitsMet <- metabolomicsMetadata %>%
  mutate(., "13ºC at Day 4" = case_when(treatment_day == "13_3" ~ 1,
                                        treatment_day != "13_3" ~ 0)) %>%
  mutate(., "5ºC at Day 4" = case_when(treatment_day == "5_3" ~ 1,
                                       treatment_day != "5_3" ~ 0)) %>%
  mutate(., "30ºC at Day 4" = case_when(treatment_day == "30_3" ~ 1,
...
rownames(datTraitsMet) <- datTraitsMet$crab.ID #Convert crab.ID into rownames.
datTraitsMet <- datTraitsMet %>%
...
- need to fix error in modTTR: double 302 and 303???? mixxing 305-308, 506
- unclear why certain things are not merging for master datTraitsMet
