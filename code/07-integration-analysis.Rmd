...
moduleTraitCorAppend <- moduleTraitCorTemp %>%
  ...
  left_join(., y = (moduleTraitCorPhys %>% 
                      ...
                      rownames_to_column(., var = "ME"))) %>%
  column_to_rownames(., var = "ME") %>%
  dplyr::select("5ºC", "13ºC", "30ºC", "Average TTR", "Oxygen Consumption") %>%
  as.matrix(.) #Take previous correlations and convert to a dataframe. Convert rownames to ME column. Join with phys correlations and reconvert column to rownames. Reorder columns to temperatures go from coldest to warmest. Convert back to a matrix
