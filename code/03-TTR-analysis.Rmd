...
TTRmodel <- glmer( #Generalized linear mixed effect model
  TTRavg ~ as.factor(treatment)*day.sc + #Treatment, day, and their interaction
    as.factor(sex) + integument.sc + carapace.width.sc + as.factor(missing.legs) + #Demographic variables: sex, integument color, carapace width, and missing legs
...
```{r}
summary(TTRmodel) #Get model summary output
```
...
```{r}
TTRmodel2 <- glm(log(TTRavg) ~ as.factor(treatment) + day + weight, data = modTTR) #Run the best fit model
