...
- Test Inverse Gaussian and Tweedie families in glmmTMB.
- Test adding a quadratic time term I(day.sc^2) to TTRmodelGamma.
- If none of these alternatives significantly improve the AIC or residual plots, stick with TTRmodelGamma. Its QQ plot is nearly linear, its dispersion test is non-significant ($p = 0.052$), and minor residual curvature in DHARMa is widely accepted in GLMM ecology and physiology literature.
...
```{r}
AIC(TTRmodelLogNorm5, TTRmodelGamma)
```

```{r}
summary(TTRmodel5) #Summary output of the final model
```

## Check assumptions

```{r}
vif(TTRmodel4) # Check multicollinearity assumption.

#Values < 5 are ideal, assumption is met
```

```{r}
sim_residuals <- simulateResiduals(fittedModel = TTRmodel5, n = 1000, plot = TRUE) #Simulate residuals for the final model
...
# 4. Check for Overdispersion / Outliers explicitly
testDispersion(sim_residuals)
