...
```{r}
write.csv(tidy(TTRmodel), "TTR-ANOVA-output.csv", quote = FALSE, row.names = FALSE) #Save table version of ANOVA output
```
...
```{r}
write.csv(tidy(TukeyHSD(TTRmodel, "treatment", conf.level = 0.95)), "TTR-ANOVA-treatment-Tukey.csv", quote = FALSE, row.names = FALSE) #Save TukeyHSD output
```
...
```{r}
write.csv(tidy(pairwise.t.test(x = log(modTTR$TTRavg), modTTR$day, p.adjust.method = "bonferroni")), "TTR-ANOVA-day-pairwiset.csv", quote = FALSE, row.names = FALSE) #Save pairwise t-test output
```
...
```{r}
write.csv(tidy(TTRthreshModel2), "TTR-binomialModel.csv", quote = FALSE, row.names = FALSE) #Save binomal GLM output
```
