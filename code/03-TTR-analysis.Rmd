...
```{r}
TTRmodel <- glmmTMB( #Generalized linear mixed effect model
...
#Dropping number.swimmers.sc decreases AIC the most
```

```{r}
#Rerun the model without number of swimmers

TTRmodel2 <- glmmTMB( #Generalized linear mixed effect model
  TTRavg ~ as.factor(treatment)*day.sc + #Treatment, day, and their interaction
...
```{r}
drop1(TTRmodel2) #Use a backwards deletion approach and AIC to identify best fit model. 

#Dropping carapace.width.sc will reduce AIC the most
```

```{r}
#Rerun the model without carapace width

TTRmodel3 <- glmmTMB( #Generalized linear mixed effect model
  TTRavg ~ as.factor(treatment)*day.sc + #Treatment, day, and their interaction
...
```{r}
drop1(TTRmodel3) #Use a backwards deletion approach and AIC to identify best fit model. 

#Dropping integument.sc will reduce AIC the most
```

```{r}
TTRmodel4 <- glmmTMB( #Generalized linear mixed effect model
  TTRavg ~ as.factor(treatment)*day.sc + #Treatment, day, and their interaction
    as.factor(sex) + #Demographic variables: sex, integument color
    (1 | treatment.tank), #Use tank as a random effect to account for potential dependence between data
  data = modTTR,
  family = Gamma(link = "log") #Gamma model with a log link
)
...
```{r}
drop1(TTRmodel4) #Use a backwards deletion approach and AIC to identify best fit model. 
...
```{r}
summary(TTRmodel4) #Summary output of the final model
```
...
```{r}
sim_residuals <- simulateResiduals(fittedModel = TTRmodel4, n = 1000, plot = TRUE) #Simulate residuals for the final model
...
```
```{r}
testDispersion(sim_residuals) #Testing dispersion

#Dispersion parameter is 0.36977, which is quite low. May not be a big issue?
```
