...
transMetabData %>%
  dplyr::select(., c(1:4, 
                     `glucose-6-phosphate`, `glucose-1-phosphate`, `glucose `, `fructose-6-phosphate `, `fructose-1-phosphate`, `pyrophosphate `, phosphate, `3-phosphoglycerate`)) %>%
  pivot_longer(., cols = c(5:12), names_to = "metabolite", values_to = "value") %>%
...
```{r}
transMetabData %>%
  dplyr::select(., c(1:4, `citric acid`, `fumaric acid`, `alpha-ketoglutarate`, `malic acid`)) %>%
...
transMetabData %>%
  dplyr::select(., c(1:4,
                     `glucose-6-phosphate`, `glucose-1-phosphate`, `glucose `, `lactic acid`)) %>%
  pivot_longer(., cols = c(5:8), names_to = "metabolite", values_to = "value") %>%
  mutate(., treatment_day = gsub(pattern = "_3", replacement = "_03", x = treatment_day)) %>%
  mutate(., treatment_day = gsub(pattern = "5_", replacement = "05_", x = treatment_day)) %>%
  ggplot(., mapping = aes(x = treatment_day, y = value, color = treatment_day)) +
...
```{r}
ggplot(KEGG_pathways, aes(x = Module, y = KEGG)) + 
  geom_point(aes(size = hits)) +
  scale_size(name = "Metabolites",
             range = c(4,15), 
             limits = c(1,5)) +
