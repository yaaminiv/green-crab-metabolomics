...
library(pheatmap)
library(genefilter)
# library(WGCNA) #Error: no package called preprocessCore
library(dendsort)
...
```{r}
dissim.metab <- vegdist(scale(transMetabData[c(6:602)]), "euclidean") #Create euclidean dissimilarity matrix to use for perMANOVA and PERMDISP
```

Since sex was a significant factor in explaining TTR differences, I want to see if sex also explains differences in metabolite abundance.

```{r}
permanova.metab.sex <- adonis2(dissim.metab ~ as.factor(treatment)*as.numeric(day) + as.factor(sex) + as.factor(treatment):as.factor(sex), data = transMetabData, method = 'eu', by = "terms") #Conduct global PERMANOVA to assess significance of trends in PCA. Scale metabolomics data and assess influence of treatment, day, and treatment x day on metabolite profiles. Include "by = "terms"" to ensure test assess significance for each term.
permanova.metab.sex
```

```{r}
broom::tidy(permanova.metab.sex) %>%
...
anova(disp.metab.day) #Variance is the same between groups. Day significance is due to centroid differences!
```
...
```{r}
dissim.metabid <- vegdist(scale(transMetabData[c(6:157)]), "euclidean") #Create euclidean dissimilarity matrix to use for perMANOVA and PERMDISP
```

```{r}
permanova.metabid.sex <- adonis2(dissim.metabid ~ treatment*day + as.factor(sex) + as.factor(treatment):as.factor(sex), data = transMetabData, method = 'eu', by = "terms") #Conduct global PERMANOVA to assess significance of trends in PCA. Scale metabolomics data and assess influence of treatment, day, and treatment x day on metabolite profiles
permanova.metabid.sex #Significant influence of all factors
```

```{r}
broom::tidy(permanova.metabid.sex) %>%
...
t.test_30v5_day3 %>%
  ...
  left_join(., (rawMetabolomicsData %>% 
                  dplyr::rename(metabolite = BinBase.name)), 
            by = "metabolite") %>%
  dplyr::select(., metabolite:p.adj, PubChem, KEGG) %>%
  unique(.) %>%
...
t.test_30v5_day22 %>%
  ...
  dplyr::select(., metabolite:p.adj, PubChem, KEGG) %>%
  unique(.) %>%
  write.table("metaboanalyst/30v5-day22_VIP_metabolites.txt", sep = "\t", quote = FALSE, row.names = FALSE) #Take significant pairwise VIP data and join with lipid identifiers. Retain unique entries and write table
```
...
t.test_30v5_day22 %>%
  ...
  dplyr::select(., metabolite:p.adj, PubChem, KEGG) %>%
  unique(.) %>%
  write.table("metaboanalyst/unique-30v5-day22_VIP_metabolites.txt", sep = "\t", quote = FALSE, row.names = FALSE) #Take significant pairwise VIP data and join with lipid identifiers. Retain unique entries and write table
```
...
```{r}
loadingsPC1 <- get_loadings(metabolomics.ASCAmodel, component = 1, effect = 1) #Get loadings for the top 10 positively and negatively loaded compounds on the PC
head(loadingsPC1)
```
...
loadingsPC1[[1]] %>%
  ...
  unique(.) %>%
  nrow(.) #25 compounds explain 50% of the cumulative variance in this PC
```
...
loadingsPC1[[1]] %>%
  ...
  dplyr::select(., PC, metabolite, loading, cum_var, PubChem, KEGG) %>%
  unique(.) %>%
  write.table("ASCA/PC1_cumvar50_metabolites.txt", sep = "\t", quote = FALSE, row.names = FALSE) #Save list of metabolites to use with Metaboanalyst
```

The file created above was put into MetaboAnalyst.

```{r}
read.csv("ASCA/PC1_metaboanalyst-results/name_map.csv") %>%
  filter(., Comment == 0) %>%
  nrow(.) #2 compounds unmatched, 23 total compounds used for analysis
```
...
```{r}
loadingsPC2 <- get_loadings(metabolomics.ASCAmodel, component = 2, effect = 1) #Get loadings for the top 10 positively and negatively loaded compounds on the PC
head(loadingsPC2)
```
...
loadingsPC2[[1]] %>%
  ...
  dplyr::select(., PC, metabolite, loading, cum_var, PubChem, KEGG) %>%
  unique(.) %>%
  nrow(.) #20 compounds explained 50% of the cumulative variance in this PC
```
...
loadingsPC2[[1]] %>%
  ...
  dplyr::select(., PC, metabolite, loading, cum_var, PubChem, KEGG) %>%
  unique(.) %>%
  write.table("ASCA/PC2_cumvar50_metabolites.txt", sep = "\t", quote = FALSE, row.names = FALSE) #Save list of metabolites to use with Metaboanalyst
```

The file created above was put into MetaboAnalyst.

```{r}
read.csv("ASCA/PC2_metaboanalyst-results/name_map.csv") %>%
  filter(., Comment == 0) %>%
  nrow(.) #0 compounds unmatched, 23 total compounds used for analysis
```

There were no significant pathways in this PC.
...
```{r}
loadingsPC3 <- get_loadings(metabolomics.ASCAmodel, component = 3, effect = 1) #Get loadings for the top 10 positively and negatively loaded compounds on the PC
head(loadingsPC3)
```
...
loadingsPC3[[1]] %>%
  ...
  dplyr::select(., PC, metabolite, loading, cum_var, PubChem, KEGG) %>%
  unique(.) %>%
  nrow(.) #16 compounds explained 50% of the cumulative variance in this PC
```
...
loadingsPC3[[1]] %>%
  ...
  dplyr::select(., PC, metabolite, loading, cum_var, PubChem, KEGG) %>%
  unique(.) %>%
  write.table("ASCA/PC3_cumvar50_metabolites.txt", sep = "\t", quote = FALSE, row.names = FALSE) #Save list of metabolites to use with Metaboanalyst
```

The file created above was put into MetaboAnalyst.

```{r}
read.csv("ASCA/PC3_metaboanalyst-results/name_map.csv") %>%
  filter(., Comment == 0) %>%
  nrow(.) #1 compounds unmatched, 15 total compounds used for analysis
```
...
```{r}
loadingsPC4 <- get_loadings(metabolomics.ASCAmodel, component = 4, effect = 1) #Get loadings for the top 10 positively and negatively loaded compounds on the PC
head(loadingsPC4)
```
...
loadingsPC4[[1]] %>%
  ...
  dplyr::select(., PC, metabolite, loading, cum_var, PubChem, KEGG) %>%
  unique(.) %>%
  nrow(.) #17 compounds explained 50% of the cumulative variance in this PC
```
...
loadingsPC4[[1]] %>%
  ...
  dplyr::select(., PC, metabolite, loading, cum_var, PubChem, KEGG) %>%
  unique(.) %>%
  write.table("ASCA/PC4_cumvar50_metabolites.txt", sep = "\t", quote = FALSE, row.names = FALSE) #Save list of metabolites to use with Metaboanalyst
```

The file created above was put into MetaboAnalyst.

```{r}
read.csv("ASCA/PC4_metaboanalyst-results/name_map.csv") %>%
  filter(., Comment == 0) %>%
  nrow(.) #1 compounds unmatched, 16 total compounds used for analysis
```
...
topPathwayResultsPC4 <- read.csv("ASCA/PC4_metaboanalyst-results/pathway_results.csv") %>%
  ...
  mutate(., negLogFDR = -log(FDR)) %>%
  arrange(desc(enrichmentRatio)) %>%
  mutate(., order = rev(1:22)) #Import MetaboAnalyst results and modify for figure
...
```{r}
predictionPC1 <- plot(metabolomics.ASCAmodel, effect = 1, component = 1, type = 'prediction', 
                      n_limit = 10, 
                      ribbon = TRUE,
                      x_angle = 0, x_h_just= 0.5,
                      flip_axis = TRUE,
                      ...
                      group_label = "Temperature (ºC)") +
  ggtitle("A. PC 1") +
  theme_classic() + theme(strip.text.x = element_text(size = 5, color = "black"))

predictionPC2 <- plot(metabolomics.ASCAmodel, effect = 1, component = 2, type = 'prediction', 
                      n_limit = 10, 
                      ribbon = TRUE,
                      x_angle = 0, x_h_just= 0.5,
                      flip_axis = TRUE,
...
predictionPC3 <- plot(metabolomics.ASCAmodel, effect = 1, component = 3, type = 'prediction', 
                      n_limit = 10, 
                      ribbon = TRUE,
                      x_angle = 0, x_h_just= 0.5,
                      flip_axis = TRUE,
...
predictionPC4 <- plot(metabolomics.ASCAmodel, effect = 1, component = 4, type = 'prediction', 
                      n_limit = 10, 
                      ribbon = TRUE,
                      x_angle = 0, x_h_just= 0.5,
                      flip_axis = TRUE,
                      ...
                      group_label = "Temperature (ºC)") +
  ggtitle("D. PC 4") +
  theme_classic() + theme(strip.text.x = element_text(size = 4, color = "black"))

predictionPC1 + predictionPC2 + predictionPC3 + predictionPC4 + plot_layout(guides = 'collect')
ggsave("../../supplementary-material/metabolite-ASCA-prediction-plot.pdf", width = 11, height = 8.5)
```

# Enrichment figure

I want to make a multipanel figure that depicts abundances of metabolites associated with significantly enriched pathways.

```{r}
#Create lists of metabolites involved in significant pathways. Some names have spaces afterwards to reflect naming conventions in original dataframe
valineBiosynthesis <- c("threonine", "isoleucine ", "leucine", "valine ") #13 vs 30 VIP, ASCA PC 1. Valine is only present in the VIP contrast
arginineBiosynthesis <- c("alpha-ketoglutarate", "glutamate ", "citrulline") #ASCA PC 4
phenylalanineBiosynthesis <- c("phenylalanine", "tyrosine") #ASCA PC 4
```

```{r}
valineBiosynthesisFigure <- transMetabData %>%
  dplyr::select(., c(1:4, any_of(valineBiosynthesis))) %>%
  pivot_longer(., cols = c(5:8), names_to = "metabolite", values_to = "value") %>%
  mutate(., treatment_day = gsub(pattern = "_3", replacement = "_03", x = treatment_day)) %>%
  mutate(., treatment_day = gsub(pattern = "5_", replacement = "05_", x = treatment_day)) %>%
  ggplot(., mapping = aes(x = treatment_day, y = value, color = treatment_day, fill = treatment_day)) +
  facet_wrap(~metabolite, scales = "free_y", nrow = 1) +
  ylab(NULL) +
  geom_boxplot(width = 0.5, position = position_dodge(width = 0.5), alpha = 0.1) +
  geom_point(pch = 21, position = position_dodge(width = 0.5)) +
  scale_fill_manual(name = "", 
                    values = c("lightblue", "#2171B5", "grey80", "#525252", "salmon", "#CB181D"),
                    labels = c("5ºC at Day 4", "5ºC at Day 22", "13ºC at Day 4", "13ºC at Day 22", "30ºC at Day 4", "30ºC at Day 22")) +
  scale_colour_manual(name = "", 
                      values = c("lightblue", "#2171B5", "grey80", "#525252", "salmon", "#CB181D"),
                      labels = c("5ºC at Day 4", "5ºC at Day 22", "13ºC at Day 4", "13ºC at Day 22", "30ºC at Day 4", "30ºC at Day 22")) +
  xlab("") + #Axis titles
  ggtitle("A. Valine, Leucine, and Isoleucine Biosynthesis") +
  theme_classic(base_size = 15) + theme(plot.title = element_text(size = 15), 
...
```{r}
arginineBiosynthesis_facets <- c("1" = "\u03B1-ketoglutarate",
                                 "2" = "glutamate",
                                 "3" = "citrulline") #Assign labels for facets
arginineBiosynthesis_facets
```

```{r}
arginineBiosynthesisFigure <- transMetabData %>%
  dplyr::select(., c(1:4, any_of(arginineBiosynthesis))) %>%
  pivot_longer(., cols = c(5:7), names_to = "metabolite", values_to = "value") %>%
  mutate(., treatment_day = gsub(pattern = "_3", replacement = "_03", x = treatment_day)) %>%
  mutate(., treatment_day = gsub(pattern = "5_", replacement = "05_", x = treatment_day)) %>%
  mutate(., order = case_when(metabolite == "alpha-ketoglutarate" ~ 1,
                              metabolite == "glutamate " ~ 2,
                              metabolite == "citrulline" ~ 3)) %>%
  ggplot(., mapping = aes(x = treatment_day, y = value, color = treatment_day, fill = treatment_day)) +
  facet_wrap(~order, scales = "free_y", labeller = labeller(order = arginineBiosynthesis_facets), nrow = 1) +
  ylab(NULL) +
  geom_boxplot(width = 0.5, position = position_dodge(width = 0.5), alpha = 0.1) +
  geom_point(pch = 21, position = position_dodge(width = 0.5)) +
  scale_fill_manual(name = "", 
  ...
  scale_colour_manual(name = "", 
                      ...
                      labels = c("5ºC at Day 4", "5ºC at Day 22", "13ºC at Day 4", "13ºC at Day 22", "30ºC at Day 4", "30ºC at Day 22")) +
  xlab("") + #Axis titles
  ggtitle("B. Arginine Biosynthesis") +
  theme_classic(base_size = 15) + theme(plot.title = element_text(size = 15), 
                                        axis.text.x = element_blank(), 
                                        axis.ticks.x = element_blank(),
                                        legend.position = "bottom", 
...
```{r}
phenylalanineBiosynthesisFigure <- transMetabData %>%
  dplyr::select(., c(1:4, any_of(phenylalanineBiosynthesis))) %>%
  pivot_longer(., cols = c(5:6), names_to = "metabolite", values_to = "value") %>%
  mutate(., treatment_day = gsub(pattern = "_3", replacement = "_03", x = treatment_day)) %>%
  mutate(., treatment_day = gsub(pattern = "5_", replacement = "05_", x = treatment_day)) %>%
  ggplot(., mapping = aes(x = treatment_day, y = value, color = treatment_day, fill = treatment_day)) +
  facet_wrap(~metabolite, scales = "free_y") +
  ...
  scale_colour_manual(name = "", 
                      ...
                      labels = c("5ºC at Day 4", "5ºC at Day 22", "13ºC at Day 4", "13ºC at Day 22", "30ºC at Day 4", "30ºC at Day 22")) +
  xlab("") + #Axis titles
  ggtitle("C. Phenylalanine, Tyrosine, and Tryptophan Biosynthesis") +
  theme_classic(base_size = 15) + theme(plot.title = element_text(size = 15), 
                                        axis.text.x = element_blank(), 
                                        axis.ticks.x = element_blank(),
                                        legend.position = "bottom", 
...
```{r}
#Create lists of metabolites involved in pathways. Some names have spaces afterwards to reflect naming conventions in original dataframe
streamlinedTCA <- c("lactic acid", "alpha-ketoglutarate", "fumaric acid", "malic acid") #Lactic acid + 3 VIP metabolites also involved in TCA
```

```{r}
streamlinedTCA_facets <- c("1" = "lactic acid",
                           "2" = "\u03B1-ketoglutarate",
                           "3" = "fumaric acid",
                           "4" = "malic acid") #Assign labels for facets
streamlinedTCA_facets
```

```{r}
streamlinedTCAFigure <- transMetabData %>%
  dplyr::select(., c(1:4, any_of(streamlinedTCA))) %>%
  pivot_longer(., cols = c(5:8), names_to = "metabolite", values_to = "value") %>%
  mutate(., treatment_day = gsub(pattern = "_3", replacement = "_03", x = treatment_day)) %>%
  mutate(., treatment_day = gsub(pattern = "5_", replacement = "05_", x = treatment_day)) %>%
  mutate(., order = case_when(metabolite == "lactic acid" ~ 1,
                              metabolite == "alpha-ketoglutarate" ~ 2,
                              metabolite == "fumaric acid" ~ 3,
                              metabolite == "malic acid" ~ 4)) %>%
  ggplot(., mapping = aes(x = treatment_day, y = value, color = treatment_day, fill = treatment_day)) +
  facet_wrap(~order, scales = "free_y", labeller = labeller(order = streamlinedTCA_facets), nrow = 1) +
  ylab(NULL) +
  geom_boxplot(width = 0.5, position = position_dodge(width = 0.5), alpha = 0.1) +
  geom_point(pch = 21, position = position_dodge(width = 0.5)) +
  scale_fill_manual(name = "", 
  ...
  scale_colour_manual(name = "", 
                      ...
                      labels = c("5ºC at Day 4", "5ºC at Day 22", "13ºC at Day 4", "13ºC at Day 22", "30ºC at Day 4", "30ºC at Day 22")) +
  xlab("") + #Axis titles
  ggtitle("D. Anaerobic and Aerobic Metabolism") +
  theme_classic(base_size = 15) + theme(plot.title = element_text(size = 15), 
                                        axis.text.x = element_blank(), 
                                        axis.ticks.x = element_blank(),
                                        legend.position = "bottom", 
