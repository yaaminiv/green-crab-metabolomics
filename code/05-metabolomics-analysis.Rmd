...
valineBiosynthesisFigure <- transMetabData %>%
  ...
  scale_fill_manual(name = "", 
                    values = c("lightblue", "#2171B5", "grey80", "#525252", "salmon", "#CB181D"),
                    labels = c("5ºC at Day 3", "5ºC at Day 22", "13ºC at Day 3", "13ºC at Day 22", "30ºC at Day 3", "30ºC at Day 22")) +
...
```{r}
(valineBiosynthesisFigure / arginineBiosynthesisFigure / phenylalanineBiosynthesisFigure) + #Combine figures vertically
  plot_layout(guides = "collect") & #Collect identical legends
  theme(legend.position = "bottom", #Have legend on the bottom
        legend.key.width = unit(0.3, "cm")) & #Reduce legend spacing
  guides(
    fill = guide_legend(nrow = 1), #Force legend to be on one row
    color = guide_legend(nrow = 1) #Force legend to be on one row
  )
ggsave("ASCA/figures/enriched-pathways-metabolite-abundance.pdf", width = 8.5, height = 11) #Save figure
```
