...
```{r}
write.csv(mME_meta, "module-expression.csv", quote = FALSE, row.names = FALSE) #Save module expression dataframe
```

First, I'm going to look at boxplots for eigengene expression by treatment x day.

```{r, warning = FALSE, message = FALSE}
#pdf("figures/eigengene-expression-boxplot.pdf", height = 8.5, width = 11)
mME_meta %>%
...
```{r}
#pdf("figures/eigengene-expression-dotplot.pdf", height = 8.5, width = 11)
mME_meta %>%
...
```{r}
write.csv(metab_rel_change, "metabolites_relative_change.csv", quote = FALSE, row.names = FALSE)
```

# Pathway analysis

The final step in this workflow is to understand the various metabolic pathways represented in each module eigengene. I'll conduct the majority of this analysis with the MetaboAnalyst GUI interface.
...
module0_metabolites <- module_df %>%
  ...
  filter(., Module == 0) %>%
  dplyr::select(., Metabolite) #Take dataframe with modules and metabolites, change colors to Module, and select metabolites for a given module. Keep only the metabolite column
nrow(module0_metabolites) #40 metabolites
write.csv(module0_metabolites, "module0_metabolites.csv", quote = FALSE, row.names = FALSE)
```
...
write.csv(module1_metabolites, "module1_metabolites.csv", quote = FALSE, row.names = FALSE)
```
...
write.csv(module2_metabolites, "module2_metabolites.csv", quote = FALSE, row.names = FALSE)
```
...
write.csv(module3_metabolites, "module3_metabolites.csv", quote = FALSE, row.names = FALSE)
```
...
write.csv(module4_metabolites, "module4_metabolites.csv", quote = FALSE, row.names = FALSE)
```
...
write.csv(module5_metabolites, "module5_metabolites.csv", quote = FALSE, row.names = FALSE)
```
...
write.csv(module6_metabolites, "module6_metabolites.csv", quote = FALSE, row.names = FALSE)
```
...
write.csv(module7_metabolites, "module7_metabolites.csv", quote = FALSE, row.names = FALSE)
```
...
write.csv(module8_metabolites, "module8_metabolites.csv", quote = FALSE, row.names = FALSE)
```
