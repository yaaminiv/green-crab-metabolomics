...
```{r}
write.csv(compound_classes, "metaboanalyst/msea-compoundClass/enrichment-output/all-module-compound-classes.csv", quote = FALSE, row.names = FALSE) #Save dataframe with all compound classes for all modules
```
...
```{r}
write.csv(sig_classes_comp, "metaboanalyst/msea-compoundClass/enrichment-output/significant-compound-classes.csv", quote = FALSE, row.names = FALSE)
```
...
ggplot(sig_classes_comp, aes(x = Module, y = compoundClass)) + 
  ...
  theme_classic(base_size = 15) + theme(axis.text.x = element_text(angle = 35, hjust = 1)) #Plot significantly enriched compound classes for each module. Scale point size by the number of metabolite hits in each category. Define x- and y- axis labels. Change the angle of the x-axis text.
