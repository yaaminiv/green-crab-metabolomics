...
I need to know how long it took for temperature to go from 13ºC to 30ºC, and from 13ºC to 5ºC.

Row 598 is when the ramping period starts for both treatments, so I'll use that to start the ramp. Row 631 is when HOBO loggers are recording data on the first full day of the experiment after the temperatures have reached the treatment setpoints, so I'll use that as my cutoff.

```{r}
rampTempStats <- data.frame("tank" = seq(1:3),
                            "initialTemp" = rep(0, times = 3),
                            "finalTemp" = rep(0, times = 3)) #Create an empty dataframe
```

```{r}
for (i in 1:(length(hobos)/4)) {
  rampTempStats[i,2] <- mean(rbind(hobos[[i]][,2][598], 
                                   hobos[[i+3]][,2][598],
                                   hobos[[i+6]][,2][598],
                                   hobos[[i+9]][,2][598]), 
                             na.rm = TRUE)
  rampTempStats[i,3] <- mean(rbind(hobos[[i]][,2][631], 
                                   hobos[[i+3]][,2][631],
                                   hobos[[i+6]][,2][631],
                                   hobos[[i+9]][,2][631]), 
                             na.rm = TRUE)
} #For each treatment, combine the  temperature data from all HOBO loggers for the start and end of the temperature ramp.
head(rampTempStats) #Confirm calculations
```

```{r}
hobos[[1]][,1][598] #Time the ramp started
hobos[[1]][,1][631] #Time the ramp ended
hobos[[1]][,1][631] - hobos[[1]][,1][598] #Time difference of 8.25 hours
```

```{r}
rampTempStats <- rampTempStats %>%
  mutate(., tempDiff = finalTemp - initialTemp, #Calculate the difference between the final and initial temperatures
         rampRate = tempDiff/8.25) #Calculate the ramp rate (ºC change/hr)
head(rampTempStats) #Confirm calculations
```

```{r}
write.csv(rampTempStats, "ramp-temp-stats.csv", quote = FALSE) #Save output
```
...
for (i in 1:(length(hobos)/2)) {
  treatmentTempStats[i,2] <- mean(rbind(hobos[[i]][,2][631:length(hobos[[i]][,2])], 
                                        hobos[[i+6]][,2][631:length(hobos[[i+6]][,2])]), 
                                  na.rm = TRUE)
  treatmentTempStats[i,3] <- sd(rbind(hobos[[i]][,2][631:length(hobos[[i]][,2])], 
                                      hobos[[i+6]][,2][631:length(hobos[[i+6]][,2])]), 
                                na.rm = TRUE)
...
tempDataLong <- rbind(tank1a, tank1b,
                      ...
                      tank4a, tank4b,
                      tank5a, tank5b,
                      tank6a, tank6b) %>% #Create long dataframe for ANOVA
...
```{r}
tempANOVA <- aov(logTemp ~ treatment, data = tempDataLong, var.equal = FALSE) #One-way ANOVA by treatment
...
```{r}
leveneTest(tempANOVA) #P < 0.05, so there are different variances between temperature treatments (makes sense given experimental design)
```

### Normality

```{r}
tempANOVAResiduals <- residuals(tempANOVA) #Calculate residuals
hist(tempANOVAResiduals) #Left-skewed distribution corrected with log transformation
qqnorm(tempANOVAResiduals)
qqline(tempANOVAResiduals) #Somewhat normally distributed
```


### Welch test

Since the homogeneity of variance assumption was violated, I will repeat the analysis with a Welch test instead of the ANOVA.

```{r}
tempWelch <- oneway.test(logTemp ~ treatment, data = tempDataLong, var.equal = FALSE) #One-way ANOVA by treatment
tempWelch # F = 30919 and P-value <  2.2e-16
```
