...
```{r}
hist(-(dat20220708_MO2_all_slope_results$slope_nmol_hr_corrected)) #Check distribution
hist(log(-(dat20220708_MO2_all_slope_results$slope_nmol_hr_corrected))) #Check log transformation
hist(sqrt(-(dat20220708_MO2_all_slope_results$slope_nmol_hr_corrected))) #Check sqrt transformation
```

```{r}
lm20220708 <- lm(log(-slope_nmol_hr_corrected) ~ treatment, data = dat20220708_MO2_all_slope_results)
summary(lm20220708)
```
...
```{r}
hist(-(dat20220712_MO2_all_slope_results$slope_nmol_hr_corrected)) #Check distribution
hist(log(-(dat20220712_MO2_all_slope_results$slope_nmol_hr_corrected))) #Check log transformation
hist(sqrt(-(dat20220712_MO2_all_slope_results$slope_nmol_hr_corrected))) #Check sqrt transformation
```

```{r}
lm20220712 <- lm(log(-slope_nmol_hr_corrected) ~ treatment, data = dat20220712_MO2_all_slope_results)
summary(lm20220712)
```
...
```{r}
hist(-(dat20220719_MO2_all_slope_results$slope_nmol_hr_corrected)) #Check distribution
hist(log(-(dat20220719_MO2_all_slope_results$slope_nmol_hr_corrected))) #Check log transformation
hist(sqrt(-(dat20220719_MO2_all_slope_results$slope_nmol_hr_corrected))) #Check sqrt transformation
```

```{r}
lm20220719 <- lm(log(-slope_nmol_hr_corrected) ~ treatment, data = dat20220719_MO2_all_slope_results)
summary(lm20220719)
```
...
```{r}
hist(-(dat20220726_MO2_all_slope_results$slope_nmol_hr_corrected)) #Check distribution
hist(log(-(dat20220726_MO2_all_slope_results$slope_nmol_hr_corrected))) #Check log transformation
hist(sqrt(-(dat20220726_MO2_all_slope_results$slope_nmol_hr_corrected))) #Check sqrt transformation
```

```{r}
lm20220726 <- lm(log(-slope_nmol_hr_corrected) ~ treatment, data = dat20220726_MO2_all_slope_results)
summary(lm20220726)
```
...
demTTRdata <- read.csv("../../data/time-to-right.csv", header = TRUE) %>%
  ...
  mutate(., missing.legs = case_when(number.legs < 10 ~ "Y",
                                     number.legs == 10 ~ "N")) %>%
  dplyr::select(., c(day, treatment, tank, probe_num, sex, integument.cont, weight, carapace.width, missing.legs)) #Import TTR data and modify to include only the desired demographic data and average TTR data for crabs used during respiration trials
...
```{r}
lmAllSlopeResults <- glm(log(-slope_nmol_hr_corrected) ~ as.factor(treatment)*as.numeric(day) + 
                           sex + weight + carapace.width + integument.cont + missing.legs, data = (all_MO2_slope_results_demTTRdata %>% filter(., blank_ratio < 0.10))) #Examine the influence of treatment, day, and various demographic factors on respiration rate. Remove outliers based on blank ratios prior to analysis.
...
### Postdoc tests for significant predictors

Temperature and day were significant predictors for righting response, but their interaction was not significant. I will conduct posthoc tests for both of these factors separately.

#### Treatment

The first thing I will do is explore pairwise differences between treatment. Since there was not a significant interaction, I do not need to account for day when I examine pairwise differences.

```{r}
lmAllSlopeResultsSig_treatment <- emmeans(lmAllSlopeResultsSig, pairwise ~ treatment, adjust = "tukey", data = all_MO2_slope_results_demTTRdata) #Use emmeans to obtain pairwise treatment differences at the timepoints listed. 
lmAllSlopeResultsSig_treatment$contrasts %>% broom::tidy(.) #Extract the contrast information to look at pairwise test output. Clean output using broom.
lmAllSlopeResultsSig_treatment$contrasts %>% 
  broom::tidy(.) %>%
...
I want to understand how TTR varied by timepoint within different days There is no way to do this with my original model since day was (rightfully) coded as a continuous variable. In order to get pairwise estimates, I will need to refactor day as a categorical variable, rerun the model, and then use `emmeans` on this revised model.
...
lmAllSlopeResultsSig_factor <- glm(log(-slope_nmol_hr_corrected) ~ treatment + day_factor, 
                                   data = all_MO2_slope_results_demTTRdata) #Run the original model, but use day as a factor
lmAllSlopeResultsSig_day <- emmeans(lmAllSlopeResultsSig_factor, pairwise ~ day_factor, 
                         data = all_MO2_slope_results_demTTRdata) #Rerun emmeans using day as a factor
lmAllSlopeResultsSig_day$contrasts %>% broom::tidy(.) #Extract contrasts to look at pairwise output. Clean output using broom.
lmAllSlopeResultsSig_day$contrasts %>%
  broom::tidy(.) %>%
