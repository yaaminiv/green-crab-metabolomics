...
Sex and integument color can influence respiration rates. Even though all crabs were roughly the same size for the chambers, there is still some variation that may explain differences in respiration rate. I will map this information to the slope results so I can incorporate it my statistical testing.
...
```{r}
lmAllSlopeResults <- lm(log(-slope_nmol_hr_corrected) ~ treatment*as.numeric(day), data = all_MO2_slope_results)
summary(lmAllSlopeResults) #No significant impact of treatment or day on respiration
```

```{r}
Anova(lmAllSlopeResults, type = "III")
```
...
- figure out emmeans code
- add stat information to the figure
