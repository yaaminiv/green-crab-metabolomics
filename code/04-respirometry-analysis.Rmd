...
test <- read_delim("../../data/respiration/2022-07-26-resp/2022-07-26_093345_Tank_3/2022-07-26_093345_Tank_3.txt", skip = 72, delim = "\t", col_names = FALSE) %>% 
  dplyr::select(., c(1:7, 12, 17, 21:25, 30, 35, 39:43, 48, 53, 57:61, 66, 71)) #Import data. Skip the first 72 rows
...
dat20220708 <- read_delim(files20220708, id = "tank", delim = "\t", skip = 72, col_names = FALSE) %>% 
  dplyr::select(., c(1:8, 13, 18, 20:26, 31, 36, 38:44, 49, 54)) #Import data from all files in the designated list. Specify tab-delimited files and skip the first 72 lines. There are no column names. Select data from probes 1-3 (probe 4 was never used). Use all date/time columns since some probes ran for longer than others.
...
dat20220708mod <- dat20220708 %>%
  dplyr::select(., -c(5:7, 14:16, 23:25)) %>%
  ...
  dplyr::select(., -c(date_1, date_2, date_3, 
  ...
  dplyr::select(., -c(og_temp, og_pressure))
...
grid::grid.draw(grid::textGrob("Oxygen Consumption (nmol/hr)", x = 0.01, rot = 90, gp = gpar(fontsize = 17)))
grid::grid.draw(grid::textGrob("Temperature (ºC)", y = 0.015, gp = gpar(fontsize = 17)))
...
```{r}
demTTRdata <- read.csv("../../data/time-to-right.csv", header = TRUE) %>% #Import righting response data
  dplyr::select(., -c(sampling.ID, notes)) %>% #Remove extra columns
  filter(., is.na(probe.number) == FALSE) %>% #Keep rows associated with OC crabs
  dplyr::rename(., probe_num = probe.number) %>% #Rename column
  mutate(., day = case_when(date == "7/8/22" ~ "04",
                            date == "7/12/22" ~ "08",
                            date == "7/15/22" ~ "11",
                            date == "7/19/22" ~ "15",
                            date == "7/22/22" ~ "18",
                            date == "7/26/22" ~ "22")) %>% #Create a day column
  filter(., day != "11" & day!= "18") %>% #Remove days without OC data
  mutate(., treatment = case_when(treatment.tank == "1" | treatment.tank == "4" ~ "13C",
                                  treatment.tank == "2" | treatment.tank == "5" ~ "5C",
                                  treatment.tank == "3" | treatment.tank == "6" ~ "30C")) %>% #Create a treatment column
  dplyr::rename(., tank = treatment.tank) %>% #Change column name
  mutate(., integument.cont = dplyr::recode(integument.color,
                                            ...
                                            "YO" = 2.5 , 
                                            "O" = 3, 
                                            "RO" = 3.5)) %>% #Recode integument color
  dplyr::select(., c(day, treatment, tank, probe_num, sex, integument.cont, carapace.width, number.swimmers)) #Keep columns of interest
...
all_MO2_slope_results_demTTRdata <- all_MO2_slope_results %>%
  mutate(tank = case_when(tank == "tank1" ~ 1,
                          ...
                          tank == "Tank_4" ~ 4,
                          tank == "Tank_5" ~ 5,
                          tank == "Tank_6" ~ 6)) %>% #Use case_when to include only the tank # in the tank column.
  mutate(probe_num = as.numeric(probe_num)) %>% #Convert probe_num to a numeric column
  left_join(., demTTRdata, by = c("day", "treatment", "tank", "probe_num")) %>%
  mutate(day = case_when(day == "04" ~ 4,
                         day == "08" ~ 8,
                         day == "15" ~ 15,
                         day == "22" ~ 22)) #Convert day back to a numeric variable
head(all_MO2_slope_results_demTTRdata) #Confirm dataframe creation
```

```{r}
write.csv(all_MO2_slope_results_demTTRdata, "all-slopes-demTTRdata.csv", quote = FALSE, row.names = FALSE) #Save combined dataset
```

### GLMM

I will use a generalized linear mixed effects model with tank as a random effect to account for the likelihood of repeated measurements over the course of the experiment.

```{r}
#Scale continuous variables for the GLMM

all_MO2_slope_results_demTTRdata$carapace.width.sc <- scale(all_MO2_slope_results_demTTRdata$carapace.width)
all_MO2_slope_results_demTTRdata$day.sc <- scale(all_MO2_slope_results_demTTRdata$day)
all_MO2_slope_results_demTTRdata$integument.sc <- scale(all_MO2_slope_results_demTTRdata$integument.cont)
all_MO2_slope_results_demTTRdata$number.swimmers.sc <- scale(all_MO2_slope_results_demTTRdata$number.swimmers)
```

```{r}
OCmodel <- glmmTMB( #Generalized linear mixed effect model
  -slope_nmol_hr_corrected ~ as.factor(treatment) + day.sc + as.factor(treatment):day.sc + #Treatment, day, and their interaction
    ...
    (1 | tank), #Use tank as a random effect to account for potential dependence between data
  data = all_MO2_slope_results_demTTRdata,
  family = Gamma(link = "log") #Gamma model with a log link
)
```

```{r}
drop1(OCmodel) #Use a backwards deletion approach and AIC to identify best fit model. 

#Removing as.factor(treatment):day.sc leads to the lowest AIC
```

```{r}
OCmodel2 <- update(OCmodel, . ~ . - as.factor(treatment):day.sc) #Drop as.factor(treatment):day.sc
drop1(OCmodel2) #Sex and integument color lead to equally low AIC. Dropping integument color first
```

```{r}
OCmodel3 <- update(OCmodel2, . ~ . - integument.sc) #Drop integument color
drop1(OCmodel3) #Dropping sex leads to lowest AIC
```

```{r}
OCmodel4 <- update(OCmodel3, . ~ . - as.factor(sex)) #Drop sex
drop1(OCmodel4) #Dropping carapace width leads to lowest AIC
```
```{r}
OCmodel5 <- update(OCmodel4, . ~ . - carapace.width.sc) #Drop sex
drop1(OCmodel5) #Dropping number of swimmers leads to lowest AIC
```

```{r}
OCmodel6 <- update(OCmodel5, . ~ . - number.swimmers.sc) #Drop number of swimmers
drop1(OCmodel6) #No more factors to drop!
```
...
```{r}
summary(OCmodel6) #Summary output of the final model
```

```{r}
Anova(OCmodel6, type = "III") %>% #Run type III Wald tests on final model
  broom::tidy(.) %>% #Tidy output
  write.csv(., "OCmodel6-typeIII-Wald-output.csv", quote = FALSE, row.names = FALSE) #Save output as a CSV file
```
...
```{r}
check_collinearity(OCmodel6) # Check multicollinearity assumption.
...
```{r}
sim_residuals <- simulateResiduals(fittedModel = OCmodel6, n = 1000, plot = TRUE) #Simulate residuals for the final model
```

Based on the plot, there are some significant quantile deviations, but no issues with uniformity or dispersion.

```{r}
testUniformity(sim_residuals) #Kolmogorov-Smirnov test (D = 0.14878, p-value = 0.08255)
```
...
#Dispersion = 1.2799, but not significant overdispersion (p-value = 0.334)
```
...
#Significant deviations (p-value = 0.04557), but that is not < 0.05 when rounded. This could be a minor fluctuation due to sample size, so I don't think it requires any significant changes to model structure.
```
...
#No differences in variance by treatment
```

### Normality of random effects

```{r}
re_normality <- check_normality(OCmodel6, effects = "random") # Check random effect normality
re_normality #Random effects are normally distributed (p = 0.991)
...
OCmodel6_noRE <- glmmTMB(
  -slope_nmol_hr_corrected ~ as.factor(treatment) + day.sc,
  data = all_MO2_slope_results_demTTRdata,
  family = Gamma(link = "log")
)

# Likelihood Ratio Test
anova(OCmodel6, OCmodel6_noRE) #Non-significant LRT output (chisq = 0, P = 1)

# Compare AICs
AIC(OCmodel6, OCmodel6_noRE) #Lower AIC without random effect (1336.151 vs. 1334.151)
```

```{r}
summary(OCmodel6)$varcor #Random effect associated with 3.1937e-05 SD
(3.1937e-05)^2 #Variance = 1.019972e-09
```
## Model bootstrapping

Since there are minor fluctuations in the quantile curves, I want to run a bootstrapping test on my model coefficients to confirm that my output is stable.

```{r}
boot_params <- bootstrap_parameters(OCmodel6, iterations = 1000) #Calculate bootstrapped p-values and 95% CI to on model coefficients
print(boot_params) #See results of boostrapping
```

```{r}
summary(OCmodel6)[[6]][[1]] #Original model output
```
...
summary(OCmodel6)[[6]][[1]] %>% #Original model output
  ...
  write.csv(., "OCmodel6-output.csv", quote = FALSE, row.names = FALSE) #Save table version of GLMM output with boostrapping results
```

## Posthoc tests for significant predictors

Since there were no significant interactions, I just need to understand temperature and day separately.

### Treatment

```{r}
OCmodel_EMM_temp <- emmeans(OCmodel6, #Model name
                            ~ treatment, #Significant predictor to investigate
...
OCmodel_EMM_temp #Estimated marginal means
```

```{r}
temp_EMM <- OCmodel_EMM_temp %>% #Take emmeans output
  broom::tidy(.) %>% #Tidy it
  dplyr::select(treatment, response, std.error) %>% #Keep columns of interest
  mutate(., LCL = response - std.error,
         UCL = response + std.error) #Create columns for upper and lower confidence intervals
head(temp_EMM) #Confirm formatting 
```

```{r}
pairs(OCmodel_EMM_temp) %>% #Get pairwise comparisons for treatment x day
  broom::tidy(.) %>% #Tidy output
  filter(., contrast != "30C / 5C") %>% #Remove comparisons between 30C and 5C
  dplyr::select(contrast:adj.p.value) %>% #Retain columns of interest
  write.csv(., "OCmodel6-emmeans-temp.csv", quote = FALSE, row.names = FALSE) #Save as a csv
```

### Day

To understand the impact of day on oxygen consumption rates, I need to rerun the model with day as a factor.

```{r}
all_MO2_slope_results_demTTRdata$day_factor <- as.factor(all_MO2_slope_results_demTTRdata$day) #Code day as a factor
OCmodel6_factor <- glmmTMB( #Generalized linear mixed effect model
  -slope_nmol_hr_corrected  ~ as.factor(treatment) + day_factor + #Temperature, time
    (1 | tank), #Use tank as a random effect to account for potential dependence between data
  data = all_MO2_slope_results_demTTRdata,
  family = Gamma(link = "log") #Gamma model with a log link
...
```{r}
OCmodel_EMM_day <- emmeans(OCmodel6_factor, #Model name
                           ~ day_factor, #Significant predictor to investigate
                           type = "response", #Convert back to the response scale since a log link was used
                           adjust = "bonferroni") #Use Bonferroni correction for multiple comparisons
OCmodel_EMM_day #Estimated marginal means
```

```{r}
day_EMM <- OCmodel_EMM_day %>% #Take emmeans output
  broom::tidy(.) %>% #Tidy it
  mutate(day = c(4, 8, 15, 22)) %>% #Add a column with real, not scaled dates
  dplyr::select(day, response, std.error) %>% #Keep columns of interest
  mutate(., LCL = response - std.error,
         UCL = response + std.error) #Create columns for upper and lower confidence intervals
head(day_EMM) #Confirm formatting 
```

```{r}
pairs(OCmodel_EMM_day) %>% #Get pairwise comparisons for treatment x day
  broom::tidy(.) %>% #Tidy output
  dplyr::select(contrast:adj.p.value) %>% #Retain columns of interest
  write.csv(., "OCmodel6-emmeans-day.csv", quote = FALSE, row.names = FALSE) #Save as a csv
```
...
all_MO2_slope_results_reorder <- all_MO2_slope_results %>%
  ...
  ungroup(.) #Ungroup
tail(all_MO2_slope_results_reorder) #Confirm changes
```

### Average oxygen consumption

I will plot average ± SE oxygen consumption with transparent points underneath. This will allow me to show average differences between treatments without having the specific marginal means from the model, since temperature x day was not a significant effect.

```{r}
mainOCPlot <- all_MO2_slope_results_reorder %>%
  filter(., blank_ratio < 0.10) %>%
  ggplot(., mapping = aes(x = day,
                          y = OCavgFull,
                          color = treatment, 
                          shape = treatment)) + #Define main plot aesthetics
  geom_pointrange(data = . %>% dplyr::select(day, treatment, OCavgFull, OCavgFullLow, OCavgFullHigh) %>% distinct(.),
                  aes(ymin = OCavgFullLow,
                      ymax = OCavgFullHigh), 
                  size = 1,
                  position = position_jitterdodge(jitter.width = 0.12),
  ) + #Add group means and standard errors to the plot
  geom_point(aes(x = day,
                 y = -slope_nmol_hr_corrected, #Needs to be negative to get positive rates
                 color = treatment),
             position = position_jitterdodge(jitter.width = 0.12),
             size = 1.5,
  ...
  scale_x_discrete(name = "Day",
                   ...
                   labels = c("4", "8", "15", "22")) + #Define x axis
  ylab("Oxygen Consumption (nmol/hr)") + #Define y axis
  scale_color_manual(values = c(plotColors[3], plotColors[2], plotColors[1]), #Add color scale
                     name = "Temperature (ºC)",
                     breaks = c("05C", "13C", "30C"),
                     labels = c("5", "13", "30")) +
  scale_shape_manual(values = c(15, 19, 17), #Add shape scale
                     name = "Temperature (ºC)",
                     breaks = c("05C", "13C", "30C"),
                     labels = c("5", "13", "30")) +
  theme_classic(base_size = 15) + theme(legend.position = "bottom") #Increase base font size and move the legend to the bottom

mainOCPlot

insetOCPlot <- all_MO2_slope_results_reorder %>%
  filter(., blank_ratio < 0.10) %>%
  filter(., treatment == "05C") %>% #Keep 5C data only
  ggplot(., mapping = aes(x = day,
                          y = OCavgFull,
                          color = treatment, 
                          shape = treatment)) + #Define main plot aesthetics
...
mainOCPlotWithInset <- mainOCPlot + 
  inset_element(insetOCPlot, left = 0.02, right = 0.5, bottom = 0.555+0.12, top = 0.85+0.12)

mainOCPlotWithInset
ggsave("figures/multipanel-oxygen-consumption-corrected-averages.pdf", height = 8.5, width = 11)
```

### Median oxygen consumption 
